...
The purpose of this thesis is to show how can be 
...
\section*{Smysl této práce:}
FreeIPA pou¿ívá pro ulo¿ení SUDO pravidel v LDAPu svoje schéma, které p¿iná¿í
mnoho vylep¿ení. \textbf{SSSD} ov¿em toto schéma neumí zpracovat (FreeIPA
nejprve pou¿ívala legacy sudo schema pro ulo¿ení a pak se navrhlo nové schéma).
FreeIPA musí nové schéma p¿elo¿it do legacy schématu (provádí to compat plugin),
které sssd umí zpracovat. P¿evodem se ov¿em ztrácí mnoho informací.

Jeliko¿ SSSD neumí to FreeIPA schéma, tak se musí dodate¿n¿ stahovat mnoho
informací, co¿ u rozsáhlého adresá¿e zp¿sobuje zpomalení. Úkolem není
navrhnout nové LDAP schéma pro SUDO, ale upravit SSSD daemon a SUDO tak, aby
podporovaly LDAP schéma, které pou¿ívá IPA, pop¿ to nové schéma vylep¿it.

Hodn¿ zjednodu¿en¿ bude výsledkem správný dotaz na LDAP adresá¿ na IPA serveru,
který pro daného klienta stáhne v¿echno pot¿ebné.

% ==============================================================================
\chapter{User's Privileges Administration}
\emph{cursive} - for unknown expressions .\\
\textbf{bold text} - for key words\\
\texttt{tt text} - file locations, file names\\
\\ 

{\color{red}
Administration tasks on the Linux and
Unix-like operating systems are
done by special user called \textbf{root}. Sometimes also called
\textbf{superuser}. This user has access to all system files and can execute all
the commands of the operating
system\footnote{http://www.linfo.org/root.html}. A regular user doesn't have
this privilege until these exceptions:
\begin{itemize}
	\item the user knows the root's password
	\item the user will use a special program which will give him root's privileges
\end{itemize}
}
...
cases a different user is the \emph{root}. Security policies which define
which users are allowed to execute what commands are called \emph{sudo
rules}\footnote{also reffered as sudoers in the following text}.
If an administrator wants to give a particular user a privilege to do someting then he has
to define a rule for the user first. Sudo rule has the following syntax:
...
which provides automatic syntax checking. Detailed descriptioin of configuration
...
Usualy the administrator manages multiple systems but sudo doesn't support
native way to distribute sudoers among multiple systems. Maintaining sudoers
...
That is a problem for administrators of large computer networks. The question is
how to distribute the sudoers to all computers? There are a few solutions to do
that:
\begin{enumerate} 
...
\subsection{LDAP directory service as a centralized database}
LDAP\footnote{Lightweight Directory Access Protocol} directorie service are characterized as a
"write-once-read-many-times" service. Directories are optiomalized for reading
and searching because they supposing to store kind of data that is not changed
very often\cite{ldap} (page: 26). It's eminently suitable for maintaining
data which are not changed very often. Sudo rules are set once, by administrator of
the system, but accessed by users many times. 
...
Directories can store variety of information and they are used by 
corporations to store information
about their employees, users of their networks, company information and ....
Our faculty also uses public directory to store information about the students and
their emploeyees.
...
A better solution would be put sudoers into a database that all clients would
have access to. The database should be optimalized for reading
