...
tìchto problému pou¾itím technologií FreeIPA a SSSD, které jsou pou¾ívány v Redhat
...
Od ètenáøe je ov¹em oèekávána u¾ivatelská znalost programu sudo a základní
koncepty adresáøù a LDAP protokolu.
...
èinnost z
pohledu programu sudo a dokumentuje èinnost pluginù ipa a ldap, které SUDO
...
Tento popis poslou¾il pro sestavení návrhu a implementaci, který tento
nedostatek odstraòuje. Návrhem, implementací a testováním navr¾eného øe¹ení se
zabývá kapitola \ref{ch:native_ipa_sudo_provider}.
...
\begin{enumerate}
	...
	\item Hodnoty atributu \texttt{ipaUniqueID} byly zámìrnì zkráceny kvùli
...
Tato kapitola popisuje zpùsob vyu¾ití programu sudo a serveru FreeIPA ve spojení
s démonem SSSD pro správu bezpeènostních politik identit v linuxových
prostøedích. Zabývá se mo¾nými zpùsoby distribuce tìchto politik, kterými se
sudo øídí. Popisuje a srovnává LDAP schémata, která definují sudo a FreeIPA, pro
uchování sudo pravidel v LDAP adresáøi. 

\section{Sudo} Pomocí programu sudo má administrátor mo¾nost delegovat oprávnìní
správce ostatním u¾ivatelùm bez nutnosti sdílení hesla správce. Umo¾òuje
u¾ivateli operaèního systému spustit jeden pøíkaz jako jiný u¾ivatel. Ve vìt¹inì
pøípadù je tímto u¾ivatelem právì správce linuxového systému, tj. u¾ivatel
\emph{root}. Tedy u¾ivatel, který má nejvy¹¹í oprávnìní. Jestli¾e u¾ivatel vyvolá
nìjaký pøíkaz za pomocí programu sudo, pak není vyzván k zadání hesla správce
systému, ale k zadání hesla daného u¾ivatele. \cite[Sekce 
4.4.3.2]{Security_Guide} Bezpeènostní politiky, kterými se sudo øídí, se nazývají
sudo pravidla. Nìkdy také oznaèovaná jako \emph{sudoers}. Tyto pravidla se
zapisují v následujícím formátu: 
...
\begin{enumerate}
	...
	\item Jestli¾e udìlá správce v sudoers souboru chybu, pak se program sudo nespustí. Do
		LDAP adresáøe není mo¾né ulo¾it data, která nesplòují syntaxi nativního LDAP
...
\subsection{Nativní LDAP sudo schéma} 
Sudo má vlastní podporu pro ulo¾ení sudo pravidel do LDAP adresáøe. Definuje
také schéma, kterým je nutno se øídit. Tøída \texttt{sudoRole}, která
specifikuje sudo pravidlo vypadá následovnì:
...
\begin{table}[h]
  ...
  \begin{tabular}{| l | l |} \hline
    \textbf{Mo¾ná hodnota atributu} & \textbf{Popis} \\ \hline\hline
		...
		uid\footnote{User ID} & \texttt{\#9843} \\ \hline
		skupina u¾ivatelù & \texttt{\%3bit} \\ \hline
		gid\footnote{Group ID} & \texttt{\%\#43} \\ \hline
...
ov¹em nedoporuèuje pou¾ívat a pova¾uje se jako \uv{zavr¾ený}.
...
které je mo¾no pou¾ít pro èasovì závislá pravidla. Jeliko¾ dále diskutovaný
FreeIPA server øe¹í vypr¹ení platnosti pravidel jiným zpùsobem, viz. USN plugin v sekci
\ref{ipa_sudo_schema}, nebudeme se tìmito atributy dále zabývat.
...
jedno pravidlo. V¾dy bude vybrán záznam s vìt¹í hodnotou. To odrá¾í chování
...
vyu¾ívány. Napøíklad ve Windows prostøedích je dnes roz¹íøeno øe¹ení \emph{Active
...
\begin{itemize}
	...
	\item V pøípadì, ¾e se u¾ivatel ocitne bez pøipojení k serveru, který uchovává
		sudo pravidla, pak není mo¾né sudo pou¾ít (pøípadnì lze pracovat pouze s
		lokálními pravidly definovanými v souboru \texttt{/etc/sudoers}).
	\item Pøi ¹patné konfiguraci NSS\footnote{Name Service Switch} mù¾e dojít k
		pøekrytí u¾ivatele z LDAP adresáøe s lokálním u¾ivatelem.
\end{itemize}

Jako nejvìt¹í nevýhodu lze pova¾ovat nulovou integraci mezi existujícími
...
administrátorem vytvoøené identity\footnote{tj. u¾ivatele nebo poèítaèe}.
Toto je jeden z problémù, které se sna¾í øe¹it projekt FreeIPA, kterým se zabývá
následující sekce.
...
protokol ¾ívá Kerberos, který vyu¾ívá symetrickou kryptografií pro generování
...
Sekce \ref{ldap_sudo_scheme_problems} popisovala problémy, které mohou nastat pøi
pou¾ití nativního LDAP sudo schématu z pohledu domény. Toto schéma sice mù¾e být
nahráno na IPA server, ale neposkytuje ¾ádné propojení s ji¾ existujícími
objekty. Z tìchto dùvodù se vývojáøi projektu FreeIPA rozhodli vytvoøit vlastní
schéma pro uchovávání sudo pravidel. Toto schéma umo¾òuje lep¹í
integraci s ji¾ existujícími objekty domény s konkrétními pravidly. Umo¾òuje
také daleko flexibilnìj¹í a jednodu¹¹í konfiguraci. U¾ivatel ji¾ tedy není dále
specifikován pouze jako øetìzec znakù, ale jako odkaz na ji¾ existující identitu
dané domény. 
...
záznam, popisující konkrétní sudo pravidlo, obsahuje jedineèné ID (ipaUniqueID),
své jméno (cn) a dále mù¾e obsahovat atributy specifikující samotné pravidlo.
...
\ref{table:ipa_user_attrs}. Jak je mo¾né odvodit z prvního øádku, záznamy
...
doménami funguje, je ov¹em nad rámec této bakaláøské práce.
...
Stejným zpùsobem je poté definován poèítaè na který bude pravidlo aplikováno.
Zde je navíc definován atribut \texttt{hostMask} pomocí kterého je mo¾né
specifikovat poèítaè, popø. skupinu poèítaèù, pomocí IP adresy nebo adresy sítì.
Tento atribut ov¹em v praxi není vyu¾íván. 
...
\textbf{skupiny pøíkazù}. Jak záznamy pøíkazù, tak záznamy skupin pøíkazù jsou v
samostatných kontejnerech. Jsou také definovány pomocí jiných tøíd. Atributy a
pøíklad jejich mo¾ných hodnot popisuje tabulka \ref{table:ipa_sudo_cmds_attrs}.
Záznamy pro sudo pøíkazy se nacházejí v kontejneru
\texttt{cn=sudocmds,cn=sudo,\$DC}. 
...
pravidel. Jestli¾e administrátor spravuje sudo pravidla pomocí LDAP serveru a
nativního LDAP sudo schématu. Pak pravidla, která jsou v adresáøi definována
budou v¾dy pou¾ita. Pokud by chtìl administrátor, napøíklad z dùvodu testování,
nìjaké pravidlo doèasnì zakázat, musel by jej z adresáøe odstranit. Tøída
...
jeho odstranìní z adresáøe.  Záznam ekvivalentního suda pravidla pro pravidla
...
Ka¾dý záznam na IPA serveru má operaèní atribut entryUSN\footnote{Entry Update
...
LDAP schémata, která pou¾ívá FreeIPA server je mo¾né nalézt v adresáøi
\texttt{/etc/dirsrv/slapd-instance\_name/}. Detailní popis tøíd a atributù pro
definici sudo pravidel na IPA serveru je mo¾né nalézt v souboru \texttt{65sudo.ldif}.
Schémata, která pou¾ívá FreeIPA server je mo¾né najít také na pøilo¾eném DVD,
viz. Pøíloha \ref{dvd}.
...
\section{System Security Services Daemon}
...
pak u v¹ech klientù musí být démon SSSD dostupný.  Pou¾ití SSSD pro pøístup k
sudo politikám, ulo¾ených  na vzdáleném serveru, má oproti pøístupu pomocí ldap
pluginu následující hlavní výhody: 
...
\begin{itemize}
	...
	\item \textbf{lep¹í výkon} - ka¾dé zavolání sudo utility zpùsobí jeden více
		...
		kdy u¾ivatel potøebuje zavolat sudo vícekrát za sebou. SSSD si ulo¾í
		pravidla do cache pamìti a pøi ka¾dém vyvolání sudo se nejprve hledají v
		cache. Dal¹í zpomalením u ldap pluginu mù¾e být situace, kdy pou¾ívá sudo
		více u¾ivatelù najednou. V takovém pøípadì se vytváøí nìkolik LDAP spojení.
		Zatímco SSSD pou¾ívá pouze jediné spojení s LDAP serverem
\end{itemize}
...
tento úèel je popsána v manuálové stránce \texttt{sssd-sudo} a nìkteré
konfiguraèní volby také v \texttt{sssd-ldap}.
...
mo¾né tento typ aktualizace najít jako \uv{\emph{smart refresh}}.
...
mo¾né tento typ aktualizace najít jako \uv{\emph{full refresh}}.
...
\texttt{manuálová stránka sssd.conf}}. Jestli¾e se nìkteré pravidlo na serveru
...
experimentování a ètení zdrojových kódù. Popis byl nezbytný pro úplné pochopení
zpùsobu, jakým SSSD sudo pravidla zpracovává. V poslední sekci je také popsán
hlavní problém, kterým se tato práce zabývá.
...
\emph{malloc}. Nepou¾ívá imperativní ani OOP paradigma, ale událostmi øízené
paradigma\footnote{Event-driven Programming Paradigm} programování.

U imperativního nebo OOP\footnote{Object-oriented programming} paradigmatu
programování se pøi provádìní programu postupuje od shora dolù. To ov¹em u
událostmi øízeného paradigmatu neplatí. Programy reagují na události a tyto
události poté urèují, která èást programu bude vykonávána. Takový program se
poté nachází ve dvou stavech:
...
Knihovna tevent realizuje tyto události za pomocí asynchronních funkcí.
...
zavolána jako výsledek o dokonèení události se nazývá \emph{callback}.
\cite{tevent} 
...
requests}. Jejich grafické znázornìní popisuje schéma \ref{tevent_requests}. S
tím je také spojena následující konvence: 

\begin{itemize}
	\item Se synchronní událostí jsou spojeny privátní data, které mají pøíponu
		\texttt{\_state}.
...
se její callback \uv{request\_done}. V této funkci je mo¾né zpracovat výsledky
dané události a následnì ji odstranit.  Volání asynchronních funkcí je mo¾né do
sebe také zanoøovat. Jedna asynchronní operace tak mù¾e volat jinou atd.
...
obaluje aby bylo mo¾né vyu¾it jejich vlastností. Tyto knihovny jsou poté v SSSD
oznaèovány jako SDAP a SBUS.

\section {Architektura SSSD}\label{sssd:architecture}
SSSD se skládá z nìkolika komponent (démonù). Hlavním procesem je zde
\emph{Monitor}, který je otcem v¹ech ostatních procesù. Stará se o to, aby
...
nejménì algoritmicky nároèných operací a v podstatì pouze èíst a pøedávat data z
cache pamìti SSSD. Komunikují tedy pouze s cache pamìtí nebo poskytovali dat,
...
plugin. Napøíklad provider identit mù¾e pou¾ít \emph{ldap plugin} pro pøístup k
identitám ulo¾ený v LDAP adresáøi, \emph{ad plugin} pro identity spravované
...
providery podporovány, je mo¾né nalézt v manuálové stránce \texttt{sssd.conf}.
...
pokud chtìjí danému pluginu zaslat nìjaký po¾adavek.  Jméno této funkce má v
SSSD obecnì pøíponu \textbf{handler}\footnote{napøíklad
\texttt{sdap\_sudo\_handler} je SUDO handler pro LDAP SUDO providera} a takto
bude oznaèována i v následujícím textu.  Polo¾ku pole \texttt{bet\_info} pro LDAP
SUDO providera zobrazuje obrázek \ref{fig:bet_info_sudo}.
...
Ka¾dý backend také pou¾ívá svou cache pamì», která je tvoøena LDB databází. Do této
pamìti zapisuje pouze daný backend a respondeøi z ní mohou data pouze èíst. Tato
cache pamì»
se také oznaèuje jako \emph{sysdb}.
...
bezpeènostních politikách jsou ulo¾eny na vzdáleném serveru, ke kterému
pøistupujeme pomocí démonu SSSD. Bì¾ný u¾ivatel chce zobrazit obsah souboru
\texttt{/etc/shadow}. K tomu ov¹em potøebuje vy¹¹í oprávnìní a proto se rozhodne
pou¾ít program sudo. Tuto situaci demonstruje obrázek
\ref{fig:sssd_sudo_data_flow}.

Po vyvolání programu sudo se nejprve zjistí informace o u¾ivateli, který sudo
vyvolal. Knihovna NSS po¹le po¾adavek o
informacích  NSS responderu, tj. procesu sssd\_nss. Ten nejprve zjistí zda se
informace o daném u¾ivateli nenacházejí v cache pamìti. Jestli¾e ne, pak po¾ádá
backend dané domény o aktualizaci cache pamìti. Tuto aktualizaci provede
nakonfigurovaný provider identit\footnote{identification provider, volba
\texttt{id\_provider} v \texttt{sssd.conf}}. Poté musí probìhnout autentizace
u¾ivatele. Tu zajistí stejným zpùsobem PAM\footnote{Pluggable authentication
module} responder.
...
responder. Ten hledá nejprve po¾adované pravidlo v sysdb. Jestli¾e se dané
pravidlo v sysdb nachází a je stále validní, pak nastává
\emph{cache hit}. V tomto pøípadì je pravidlo vráceno programu sudo. 

Jestli¾e hledané pravidlo v cache pamìti nadále není validní, pak je provedena
...
Jestli¾e pøi prvním dotazu do cache pamìti není po¾adované pravidlo nalezeno, pak
...
poèkat na provedení nìkteré z prùbì¾ných aktualizací, které prùbì¾nì sudo
provider. Tuto funkcionalitu blí¾e popisuje sekce
...
Jestli¾e se administrátor rozhodne ulo¾it sudo pravidla do LDAP adresáøe, tj. né
na server FreeIPA, musí
dodr¾et sudem definované schéma. Tedy pøesná jména atributù a formát hodnot.
...
\texttt{sssd.conf} specifikovány.  Jestli¾e se daná
...
mapu specifikovat. SDAP definuje
...
vrací funkce knihovny OpenLDAP, a vrací je jako ldb elementy. Zde je
nezbytné zmínit, ¾e jako první polo¾kou v mapì musí být \textbf{v¾dy}
...
sudorule mapa. Jako výchozí mapa je pou¾ita \texttt{native\_sudorule\_map} a pøi
vytváøení se také zohledòují volby zadané v konfiguraèním souboru SSSD.
Ukazatel na vytvoøenou mapu je ulo¾en v \texttt{id\_ctx}\footnote{Kontext
providera identit (angl.  Context of Identity Provider)}. Vytvoøení a pou¾ití map
je blí¾e popsáno v sekci \ref{sdap}.
...
LDAP protokolem \cite[RFC2254]{rfc2254}.  Jsou pouze doplnìny bílými znaky pro lep¹í èitelnost.
...
Pøedpokládejme pøíklad kdy je na klientském poèítaèi spu¹tìno SSSD a poèítaè má
...
která se mezi pravidly nachází je \textbf{9270}. LDAP filtr, který by se pou¾il pøi
...
ji¾ nejsou validní pravidla \textbf{rule1} a \textbf{rule2}. Poté by LDAP a
sysdb filtr pro jejich aktualizaci vypadal následovnì:
...
\ref{fig:ldap_sudo_provider_tevent_flow}. 

Ve funkci \texttt{sdap\_sudo\_refresh\_retry} probìhne kontrola zda je klient 
stále pøipojen k síti.
...
odstranìny v¹echny záznamy, které odpovídají filtru pro konkrétní typ aktualizace.  Pøi
...
\texttt{sssd.conf}}. Jeho hodnota reprezentuje èas, kdy daný záznam 
stane neplatným. Zároveò je získána nejvy¹¹í hodnota USN, která ov¹em
není ulo¾ena v cache pamìti spoleènì s pravidly, ale v kontextu
\texttt{id\_ctx->srv\_opts} v promìnné \texttt{max\_sudo\_value}.

\subsection {LDAP SUDO Handler}
Jestli¾e SUDO responder nenajde daná pravidla v sysdb, pak za¹le
SBUS po¾adavek pøes SUDO
...
\ref{sec:sssd_from_sudo_prespective}. Samotnou definici handleru je mo¾né najít v modulu
\texttt{sdap\_sudo.c}.

LDAP SUDO provideru je pøes handler pøedán typ aktualizace, který po nìm SUDO
respoder ¾ádá a pole pravidel, které chce responder aktualizovat\footnote{pole jmen pravidel, tj. pole hodnot
\texttt{cn} atributù}. SUDO responder mù¾e po¾ádat o úplnou aktualizaci
nebo o aktualizaci pravidel, která ji¾ v sysdb nejsou nadále platná.
...
sudo pluginu, který bude zaji¹»ovat podporu nativního IPA SUDO schématu. 
...
na daného klienta aplikovatelné. Odkaz na toto pravidlo se ov¹em nachází v
...
sudo schématu. Je nutné provést pøeklad sta¾ených pravidel. Pøíklad tohoto
pøekladu, který v souèasné dobì provádí compat plugin demonstruje schéma
...
a \texttt{HOST\_GROUP} není nutné rozli¹ovat, ov¹em li¹í se v prefixu, který k
pøelo¾ené hodnotì nutdno pøidat. Viz. pøíklady hodnot atributù \texttt{sudoUser}
...
atributù vèetnì pøíkladù, viz. Pøíloha \ref{dvd}. Tato tabulka byla vytvoøena na
...
webovém rozhraní serveru FreeIPA. To umo¾òuje atributu
...
pravidla ve kterých se vyskytla. Stále je ov¹em nutné N-krát iterovat
...
má být daný pøíkaz povolen/zakázán a také poøadí daných pøíkazù na kterých
...
tento pøeklad bude realizovat. Nabízí se øe¹ení ulo¾it sta¾ená sudo pravidla v
IPA formátu pøímo do sysdb a nechat sudo respondera tyto pravidla pøekládat.
...
provider. Mohlo by se tedy zdát, ¾e jediná úprava, kterou je nutno provést v
LDAP SUDO providerovi, je pøidání pøekladu pravidel pøed samotným ulo¾ením
tìchto pravidel do cache pamìti. Pøeklad pravidel je ov¹em  specifický pro IPA
providera. V kódu LDAP providera by se tedy muselo vyskytnou volání funkcí,
které by se nacházely v IPA providerovi. Tento pøístup je sice
...
providera.  Je ov¹em mo¾né z IPA providera volat kód LDAP providera, LDAP
provider je mo¾né si pøedstavit jako jakýsi stavební blok nad kterým mohou
stavìt ostatní provideøi a toho je mo¾no vyu¾ít. Vìt¹í zásah do LDAP providera
...
I kdyby tato zmìna byla provedena, stále by byl problém s odkazy na skupiny
pøíkazù. V tìchto záznamech bychom toti¾ na¹li opìt odkazy na pøíkazy, co¾ by
znamenalo dal¹í LDAP dotaz.
...
pøípadì výkonnostních problémù je mo¾no pøidat podporu ha¹ovací tabulky, která
...
vy¾adovalo úpravu ve funkci \texttt{sdap\_sudo\_refresh\_load\_done}. Jestli¾e
tento modul vyu¾ívá IPA SUDO prvoder. Pak sta¾ená pravidla je¹tì nemohou být
ulo¾ena do sysdb. Proto je asynchronní událost sta¾ení pravidel oznaèena jako
ukonèena pøed prací s cache pamìtí a øízení je vráceno zpìt IPA SUDO providerovi.
...
\textbf{pøelo¾ených} pravidel. Pro plánování pravidelných aktualizací byl pou¾ít
interní plánovaè periodických událostí \emph{ptask}. Novì navr¾ený provider se
skládá z následujících modulù:
...
\begin{itemize}
	...
	\item \texttt{ipa\_async\_sudo.c} \,--\, asynchronní sta¾ení sudo pravidel z
		IPA serveru
	\item \texttt{ipa\_async\_sudo\_cmds.c} \,--\, asynchronní sta¾ení záznamù o
		pøíkazech pro sta¾ená sudo pravidla
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\chapter{Závìr}
Tato bakaláøská práce popisuje LDAP schémata, která pou¾ívají sudo a FreeIPA k
ukládání sudo pravidel v LDAP adresáøích. Srovnává jejich rozdíly a výhody
jejich pou¾ití. Dále dokumentuje funkcionalitu ldap a ipa pluginù, které má k
dispozici SUDO provider démonu SSSD. Tato dokumentace odkrývá nedostatek démonu
SSSD, jeliko¾ nepodporuje nativní IPA SUDO schéma. Tento nedostatek se podaøilo
odstranit navr¾ením a implementací nativního IPA SUDO providera, který toto
...
testù. Pøi návrhu byla také nalezena jedna chyba a poukázáno na nìkteré
...
\begin{itemize}
	\item øetìzce v \\texttt{} se nazalomí na ¹íøku øádku!!
