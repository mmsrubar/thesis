...
Tato bakaláøská práce popisuje správu bezpeènostních politik s~pomocí
...
tìchto problémù zalo¾ená na pou¾ití technologií FreeIPA a SSSD, které jsou
pou¾ívány v~Red Hat Enterprise prostøedích.  Tato øe¹ení ov¹em vykazují urèité
nedostatky, které tato práce odkrývá. Smyslem této práce je tyto nedostatky
identifikovat, navrhnout jejich øe¹ení a tyto øe¹ení implementovat. Od ètenáøe
je v~práci oèekávána u¾ivatelská znalost programu SUDO, základních konceptù
online adresáøù a LDAP protokolu. Práce byla zadána ve spolupráci
s~firmou Red Hat.

Práce je rozdìlena do pìti kapitol. První kapitola popisuje obsah práce.
V~kapitole \ref{politics_managament} jsou popsány problémy, které vznikají pøi
centralizované správì identit a jejich bezpeènostních politik za pomoci programu
SUDO. Diskutuje také nad mo¾ným øe¹ením v~podobì pou¾ití serveru FreeIPA a
démonu SSSD. 

Tøetí kapitola se blí¾e zabývá démonem SSSD. Popisuje jeho architekturu, èinnost
z~pohledu programu SUDO a dokumentuje èinnost zásuvných modulù, ipa a ldap, které SUDO
...
Kapitola \ref{ch:native_ipa_sudo_provider} následnì prezentuje návrh, který
poslou¾il k~odstranìní nedostatku identifikovaného v~sekci 
\ref{ch:sssd:sec:ipa_provider}.

Poslední kapitola práce poté zhodnocuje dosa¾ené výsledky a diskutuje nad
mo¾nostmi pokraèování v~této práci. Tato práce pou¾ívá následující konvence:
\begin{enumerate}
	\item V¹echny dùle¾ité pojmy jsou vysázeny \textbf{tuèným písmem}. 
	\item Nové pojmy jsou vysázeny \emph{kurzivou}.
	\item Hodnoty atributu \texttt{ipaUniqueID}\footnote{Atribut je blí¾e popsán
v~sekci \ref{ipa_sudo_schema}.} byly zámìrnì zkráceny kvùli
		velikosti textu a obrázkù.
	\item V¹echny názvy souborù, jejich obsahy, u¾ivatelské úèty, klíèových slova
		jazyka C nebo jména identifikátorù jsou vysázeny \texttt{strojovým písmem}.
\end{enumerate}
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\chapter{Správa bezpeènostních politik}\label{politics_managament}
Tato kapitola popisuje mo¾nost správy bezpeènostních politik pomocí
programu SUDO. Popisuje mo¾né problémy, které mohou nastat pøi distribuci
pravidel, kterými se SUDO øídí a dále LDAP schéma, které SUDO pou¾ívá
pro ulo¾ení tìchto pravidel v~LDAP adresáøích. Zabývá se nedostatky, které
zmínìné schéma provázejí z~pohledu domény a pøedstavuje nové schéma projektu
FreeIPA, který se tyto nedostatky sna¾í øe¹it. Dále popisuje klientského démona
SSSD, který je nezbytný pro vyu¾ití tohoto schématu.

\section{SUDO} Pomocí programu SUDO má administrátor mo¾nost delegovat oprávnìní
správce\footnote{u¾ivatele \texttt{root}} ostatním u¾ivatelùm bez nutnosti sdílení hesla
správce. Tento program umo¾òuje u¾ivateli operaèního systému spustit jeden pøíkaz jako jiný
u¾ivatel. Ve vìt¹inì pøípadù je tímto u¾ivatelem právì správce linuxového
systému, neboli u¾ivatel \emph{root}, tj. u¾ivatel, který má nejvy¹¹í oprávnìní.
Jestli¾e u¾ivatel spustí nìjaký pøíkaz za pomocí programu SUDO, pak není vyzván
...
nazývají SUDO pravidla. Nìkdy jsou tato pravidla oznaèována jako \emph{sudoers}.
Tato pravidla se zapisují jako následující ètveøice: 
...
\begin{itemize}
	\item \texttt{user} \,--\, specifikuje u¾ivatele nebo skupinu, na kterou bude
		pravidlo aplikováno,
	\item \texttt{host\_list} \,--\, reprezentuje seznam koncových systémù,
...
dát napøíklad u¾ivateli \texttt{xsruba03} oprávnìní spou¹tìt pøíkaz
\texttt{/sbin/fdisk} s~právy u¾ivatele \texttt{root}, pak musí definovat následující
pravidlo: 

\begin{table}[H]
		\begin{tabular}{c}
		\label{sudoers:example}
...
správce distribuovat tyto pravidla mezi více poèítaèù. SUDO ov¹em nemá nativní
zpùsob, jak tato pravidla mezi více poèítaèù distribuovat \cite[Kapitola
20]{Identity_Management_Guide}. Spravovat tato pravidla ruènì na v¹ech
poèítaèích, které administrátor spravuje, by bylo ov¹em velice pracné a èasovì
nároèné. Otázkou zùstává jak uvedená pravidla ¹íøit mezi více poèítaèù.
Existuje nìkolik zpùsobù, jak toho docílit:

\begin{enumerate}
	\item Ruènì distribuovat pøíslu¹ná pravidla mezi více systémù za pou¾ití
		standardních linuxových nástrojù, jako jsou \emph{cron, scp, rsync, \dots}
		Tento pøístup by ov¹em mohl být èasovì nároèný a také by nemusel být
		spolehlivý.
	\item Pou¾ít speciální nástroje, jako je napøíklad
		\emph{Puppet}\footnote{http://puppetlabs.com}, který sleduje a automaticky
		¹íøí konfiguraèní soubory.  
	\item Ulo¾it SUDO pravidla do centralizované databáze.
\end{enumerate}

V~souvislosti s~bodem 3 se jeví jako výhodné pou¾ití adresáøù
LDAP\footnote{Adresáø, ke kterému se pøistupuje pomocí LDAP protokolu.}. Tyto
adresáøe mohou uchovávat rùzné typy informací a jsou tak pou¾ívány organizacemi
a institucemi k~ukládání informací o~jejich zamìstnancích, u¾ivatelích nebo
zaøízeních jejich poèítaèových sítí. Jsou charakterizovány jako slu¾ba typu 
\uv{jednou-zapsat-a-mnohokrát-èíst}\footnote{write-once-read-many-times}.
...
do LDAP adresáøe nabízí následující výhody:

\begin{enumerate}
	\item Jestli¾e jsou SUDO pravidla centralizována, pak se správce systému
		nemusí starat o~jejich distribuci. Spravuje pouze jediný soubor
s~pravidly.
	\item Vyhledávání pravidla v~LDAP adresáøi je rychlej¹í. Pøi pou¾ití
		lokálních sudoers\footnote{tj. kdy¾ jsou SUDO pravidla definována v~lokální
		\texttt{/etc/sudoers} souboru} musí SUDO pøeèíst celý soubor
		\texttt{/etc/sudores}. Pøi
	...
	\item Jestli¾e udìlá správce v~sudoers souboru chybu, pak se program SUDO nespustí. Do
		LDAP adresáøe není mo¾né ulo¾it data, která nesplòují dané schéma.
		Správná syntaxe je tedy zaruèena. Ov¹em stále je mo¾né udìlat chybu
		v~u¾ivatelském jménu, pøíkazu nebo názvu koncového systému.
\end{enumerate}
...
oznaèován jako \uv{\emph{single point of failure}}.}, na kterém jsou pravidla ulo¾ena.
V~takovém pøípadì nejsou pravidla pro sudo dostupná a není mo¾né jej vyu¾ívat.
V~praxi se tento problém dá øe¹it replikací nebo cachováním sudo pravidel.
...
atributù a tøíd \cite[Kapitola 8]{understanding_and_deploying_LDAP}. SUDO
definuje vlastní schéma, které je pou¾ito pøi vytváøení nebo vyhledávání SUDO
pravidel. Schéma definuje, které atributy je nutno pou¾ít pro ulo¾ení
prvkù\footnote{Na koho bude pravidlo aplikovatelné, kde, jaké obsahuje pøíkazy,
\dots} SUDO pravidla a tøídu, ze které jsou záznamy,
reprezentující SUDO pravidla, odvozeny. Tato tøída se nazývá \uv{sudoRole} a
její definice vypadá následovnì:
...
tabulka \ref{table:sudoUser}. SUDO pravidlo je také mo¾né aplikovat na v¹echny
...
Posledním atributem pro sestavení SUDO pravidla, jeho¾ mo¾né hodnoty popisuje tabulka
\ref{table:sudoCommand}, je atribut \texttt{sudoCommand}.
...
\begin{table}[h]
  ...
  \begin{tabular}{| l | l |} \hline
    \textbf{Mo¾ná hodnota atributu} & \textbf{Popis} \\ \hline\hline
		povolený pøíkazzakázaný pøíkaz & \texttt{/sbin/blkid /dev/sda1} \\ \hline
		zakázaný pøíkaz & \texttt{!/sbin/fdisk} \\ \hline
		v¹echny pøíkazy & \texttt{ALL} \\ \hline
  \end{tabular}
  \end{center}
\end{table}

SUDO pøíkaz je mo¾né spustit, jako jiný u¾ivatel ne¾ \texttt{root}. Specifikovat
u¾ivatele popø. skupinu, pod kterou bude moct daný pøíkaz provést, specifikují
...
ov¹em nedoporuèuje pou¾ívat a pova¾uje se za \uv{zastaralý}\footnote{deprecated}.

K~SUDO pravidlu je také mo¾no specifikovat rùzné volby, které napøíklad urèují,
...
Schéma sudo pravidel dále definuje atributy \texttt{sudoNotBefore} a
\texttt{sudoNotAfter}, které je mo¾no pou¾ít pro èasovì závislá pravidla. Pomocí
tìchto atributù je mo¾né specifikovat, po jakou dobu bude pravidlo
validní, napø. po dobu pracovní doby od 7:00 do 15:00. Tyto atributy
ov¹em aktuálnì nejsou FreeIPA serverem pou¾ívána a proto se jimi dále nebudeme
zabývat.

\label{sec:sudoers_ordering}
Posledním definovaným atributem sudo pravidel je atribut \texttt{sudoOrder}. Ten
pøedstavuje èíselnou hodnotu,  která je pou¾ita v~situaci, kdy bude mo¾no
aplikovat více ne¾ jedno pravidlo. V¾dy bude vybrán záznam s~vìt¹í hodnotou
tohoto atributu. To odrá¾í chování \uv{poslední shody} lokálního sudoers
souboru. Pøedpokládejme, ¾e soubor \texttt{/etc/sudoers} obsahuje pouze
následující dvì pravidla:
...
V~tomto pøípadì u¾ivatel \texttt{xsruba03} nebude mít oprávnìní spustit pøíkaz
\texttt{blkid}. Jestli¾e ov¹em uvedená pravidla prohodíme, pak u¾ivatel
...
Záznam ekvivalentního suda pravidla pro pravidlo \ref{sudoers:example} by v~LDAP
adresáøi vypadal tak, jak jej popisuje obrázek \ref{fig:ldap_sudoer}.

\begin{figure}[h]
	\centering
	\includegraphics[scale=1.00]{fig/sudo/ldap_sudoer.pdf}
	\caption{Pøíklad záznamu SUDO pravidla v~LDAP adresáøi.}
...
jeho jednoznaèná identifikace. V~linuxovém prostøedí se typicky jedná napø. o
u¾ivatel nebo poèítaè.  Vìt¹ina administrátorù spravujících prostøedí
o~desítkách a více u¾ivatelù chce spravovat identity centrálnì.  LDAP adresáøe
jsou pro tento úèel tedy velice vyu¾ívány. Ve Windows prostøedích je
mo¾no pou¾ít øe¹ení \emph{Active Directory}, \emph{FreeIPA}\footnote{Free
Identity, Policy and Audit} pak v~linuxových prostøedích. Na tyto servery je 
nativní LDAP SUDO schéma nahrát a zaèít tak SUDO pravidla spravovat centrálnì.
Pøístup ke správì identit a bezpeènostních politik pomocí LDAP adresáøe a
nativního SUDO schématu má ov¹em
nìkolik nevýhod: 
...
\begin{itemize}
	...
	\item V~pøípadì, ¾e se u¾ivatel ocitne bez pøipojení k~serveru, který spravuje
...
není mo¾né, s~pou¾itím nativního LDAP SUDO schématu, dosáhnout. Pøi pou¾ití
nativního schématu se pøi definici SUDO pravidel není mo¾né odkazovat na ji¾
administrátorem vytvoøené identity, tj. napøíklad u¾ivatele nebo poèítaèe.
...
identity centrálnì a zároveò spojovat identity s~autentizaèními a autorizaèními
politikami. V~linuxových prostøedích existuje mnoho protokolù pro rùzné slu¾by.
Je mo¾né napøíklad vyu¾ít NIS\footnote{Network Information Service} nebo
Kerberos+LDAP\footnote{Lightweight Directory Access Protocol} pro správu identit
a jejich autentizaci nebo pouze LDAP pro správu sudo pravidel.  ®ádné z~tìchto
slu¾eb ov¹em nejsou nijak propojeny a zároveò v¹echny musí být spravovány
lokálnì. Administrátor tedy musí mít potøebné znalosti v¹ech tìchto slu¾eb a
protokolù, aby je dokázal správnì pou¾ívat a dané slu¾by spravovat. 

Projekt FreeIPA, zkrácenì jen IPA, se zamìøuje na správu identit a
jejich politik.  Je postaven nad existujícími nativními linuxovými aplikacemi a
standardizovanými protokoly. Definuje doménu, ve které se vyskytují servery,
klienti a vzájemnì mezi sebou sdílí centrálnì spravované slu¾by.  Poskytuje
jakousi obálku nad standardizovanými sí»ovými slu¾bami, jako jsou
PAM\footnote{Pluggable Authentication Modules}, LDAP, Kerberos,
DNS\footnote{Domain Name System}, NTP\footnote{Network Time Protocol} nebo
certifikaèní slu¾by. Jeho hlavní úlohou je spravovat v¹echny tyto slu¾by na
jednom místì. IPA navíc umo¾òuje synchronizaci dat s~Active Directory.  Neumí
ov¹em Windows klienty spravovat.

\begin{figure}[h]
	\centering
	\includegraphics[scale=0.65]{fig/ipa/ipa.pdf}
	\caption{FreeIPA sjednocuje standardní slu¾by \cite{Identity_Management_Guide}.}
...
nebo klientù, IPA pou¾ívá \emph{Dogtag Certificate
System}\footnote{http://pki.fedoraproject.org/wiki/PKI\_Main\_Page}. Slu¾by jako
Kerberos jsou závislé na doménových jménech a èasových razítkách. Proto IPA
slou¾í také jako DNS a NTP server \cite[Kapitola 1]{Identity_Management_Guide}.

S ohledem na vytyèené cíle této bakaláøské práce nás dále bude zajímat pouze
správa SUDO politik.  Tedy jakým zpùsobem IPA server SUDO pravidla ukládá a jak
jsou zpracovány na stranì klienta.
...
vytvoøit vlastní schéma pro ukládání SUDO pravidel. Novì navr¾ené schéma
umo¾òuje lep¹í integraci s~ji¾ existujícími objekty s~konkrétními pravidly.
Umo¾òuje také daleko flexibilnìj¹í a jednodu¹¹í konfiguraci. U¾ivatel zde ji¾
dále není dále specifikován pouze jako øetìzec znakù, ale jako odkaz na ji¾
existující objekt dané domény. 

Pro vyjádøení vztahu mezi jednotlivými záznamy se v~adresáøích obecnì pou¾ívají
jedineèná jména záznamù adresáøe \cite[Kapitola 1]{deref}. Pro tyto jména bude
dále pou¾íváno oznaèení \emph{\textbf{DN}}\footnote{Distinguished Name}. Jestli¾e se tedy
atribut \texttt{user} odkazuje na záznam, s~informacemi o~u¾ivateli, bude
hodnota tohoto atributu tvoøena právì DN daného záznamu.  IPA SUDO schéma
definuje novou tøídu pro SUDO pravidla, jejich¾ definice vypadá následovnì:
...
atributy, napø. \texttt{memberUser} nebo \texttt{memberAllowCmd}, obsahují DN
ji¾ existujících objektù IPA domény. Pro objekty, které jsou definovány mimo IPA
doménu, se pou¾ívají \texttt{external*} atributy, napø. \texttt{externalUser} nebo
\texttt{externalHost}.  Speciální pøípady, kdy je napøíklad SUDO pravidlo
aplikovatelné na kteréhokoliv u¾ivatele nebo poèítaè specifikují
\texttt{*Category} atributy, napø. \texttt{userCategory} nebo
\texttt{cmdCategory}.
...
cn=users,cn=accounts,\$DC\footnote{Domain Controller}}. Pomocí atributu
\texttt{externalUser} je také mo¾né vytvoøit SUDO pravidlo aplikovatelné na
u¾ivatele, který není èlenem IPA domény.  Tento u¾ivatel ov¹em musí být èlenem
jiné domény, napø. domény spravováné pomocí Active Directory, a mezi
touto a IPA doménou musí být vytvoøena vzájemná dùvìra, tzv.  \emph{trust}.
Popis, jak toto propojení mezi doménami funguje nebo jak jej vytvoøit, je ov¹em
nad rámec této bakaláøské práce.
...
\begin{table}[h]
  ...
  \begin{tabular}{| l | l |} \hline
    \textbf{Jméno atributu} & \textbf{Hodnota atributu} \\ \hline\hline
		...
		\texttt{memberUser} & \texttt{uid=xsruba03,cn=users,cn=accounts,\$DC} \\ \hline
...
pravidla, jsou pøíkazy. Popsané schéma pøiná¹í novou vlastnost a tou jsou
\textbf{skupiny pøíkazù}. Jak záznamy pøíkazù, tak záznamy skupin pøíkazù jsou
v~\textbf{samostatných} kontejnerech. Jsou také definovány pomocí jiných tøíd.
Atributy a pøíklad jejich mo¾ných hodnot popisuje tabulka
\ref{table:ipa_sudo_cmds_attrs}.
...
serveru, pak pravidla, která jsou v~adresáøi definována, budou v¾dy pou¾ita.
...
\ref{sudoers:example} a \ref{fig:ldap_sudoer} v~novém IPA SUDO schématu popisuje obrázek
...
Ka¾dý záznam na IPA serveru má také \textbf{operaèní} atribut
entryUSN\footnote{Entry Update Sequence Number}. Hodnota tohoto atributu
reprezentuje èas poslední zmìny záznamu. Aktualizaci této hodnoty provádí
\emph{USN plugin} pøi ka¾dé zmìnì daného záznamu. Tento plugin poskytuje zpùsob,
který umo¾òuje informovat IPA klienty o~\textbf{modifikaci} jakéhokoliv záznamu
\cite[Sekce 3.4]{Redhat_Directory_Server_Administration_Guide}. 
...
\subsubsection{Definice SUDO pravidel} 
Dal¹í výhodou, kterou pøiná¹í FreeIPA, je snadnìj¹í definice pravidel.  SUDO
pravidla na IPA serveru je mo¾né definovat dvìma zpùsoby a to pomocí
\textbf{webového rozhraní} a jednak pomocí utilit pøíkazového øádku \texttt{ipa
...
Jestli¾e se administrátor rozhodne spravovat identity a jejich politiky
centralizovanì, napø. pomocí technologií LDAP+Kerberos, musí provést
konfiguraci NSS\footnote{Name Service Switch} a PAM a pou¾ít správné moduly
pro tuto podporu. Napøíklad \texttt{nss\_pam\_ldapd} pro pøístup k~identitám a
autentizacím na LDAP serveru.  Nebo \texttt{pam\_krb5} pro Kerberos autentizaci.
Správná konfigurace mù¾e být pro bì¾ného u¾ivatele ov¹em velice slo¾itá. Navíc, i po
správné konfiguraci, zde zùstávají dùle¾ité nedostatky tohoto øe¹ení:

\begin{enumerate}
	\item Jakmile je u¾ivatelský poèítaè odpojen od sítì, pak se u¾ivatel nemù¾e
		pøihlásit. U¾ivatel to pak øe¹í separátním u¾ivatelským úètem.
	\item Jestli¾e se u¾ivatel pøipojuje do více domén zároveò, ka¾dá mù¾e
		potencionálnì pou¾ívat jiný zpùsob autentizace a ovìøování oproti jiným
		serverùm. Bylo by tedy nutné provést novou konfiguraci.
	\item Pøi pou¾ití mnoha technologií je potøeba provádìt konfiguraci více souborù.
\end{enumerate}

Tyto nedostatky øe¹í projekt SSSD, který poskytuje pøístup k~rùzným zdrojùm,
které spravují identity, jejich autentizaci a politiky. Identity a jejich
autentizace tak mù¾e být spravována napø. pomocí technologií LDAP+Kerberos,
FreeIPA nebo Active Directory. Je také mo¾né jej pou¾ít pouze pro pøístup k~SUDO
pravidlùm spravovaných centrálnì pomocí LDAP adresáøe.  Dal¹í výhodou je, ¾e
umo¾òuje u¾ivateli pøipojení do více domén zároveò.  Jestli¾e klient støídá
prostøední napø. mezi prací a ¹kolou, nemusí stále mìnit slo¾itou konfiguraci.
SSSD pøiná¹í také offline podporu a cachování. Informace o~identitách nebo napø.
sudo pravidla jsou ukládána do cache pamìti, která se nachází na disku poèítaèe.
Tyto informace je tedy mo¾né vyu¾ívat i v~situacích, kdy se klient ocitne bez
pøipojení k~síti. To v¹e je mo¾né nakonfigurovat pomocí jediného konfiguraèního
snapøíklad ouboru \texttt{/etc/sssd/sssd.conf}. 

Jestli¾e se administrátor rozhodne spravovat SUDO politiky pomocí IPA serveru,
pak u~v¹ech klientù musí být démon SSSD dostupný.  Pou¾ití SSSD pro pøístup
k~SUDO politikám, ulo¾ených  na vzdáleném serveru, má oproti pøístupu pomocí
ldap pluginu následující hlavní výhody: 

\begin{itemize}
	\item \textbf{Offline podpora} \,--\, SSSD si ukládá v¹echna pøijatá pravidla do
		své cache pamìti. Jestli¾e dojde k~odpojení klienta od sítì, pak
		mù¾e stále pou¾ívat tyto ulo¾ená pravidla.
	\item \textbf{Lep¹í výkon} \,--\, Ka¾dé zavolání SUDO utility zpùsobí jeden
		nebo více LDAP dotazù na server, kde jsou daná pravidla ulo¾ena. V~kombinaci
		s~pomalou sítí nebo zatí¾eným serverem to mù¾e znamenat znaèné zpomalení
		v~pøípadech, kdy u¾ivatel potøebuje pou¾ít program SUDO vícekrát za sebou.
		SSSD si ulo¾í pravidla do cache pamìti a pøi spu¹tìní programu SUDO se
		pravidla hledají nejprve v~cache pamìti. Dal¹ím zpomalením u~ldap pluginu
		mù¾e být situace, kdy pou¾ívá SUDO více u¾ivatelù najednou. V~takovém
		pøípadì se vytváøí nìkolik LDAP spojení.  Zatímco SSSD pou¾ívá pouze jediné
		spojení s~LDAP serverem
\end{itemize}

SSSD také podporuje nativní LDAP SUDO schéma. Je tedy mo¾né jej nastavit tak,
aby získával SUDO pravidla z~LDAP serveru. V~takovém pøípadì získá u¾ivatel
jednak offline podporu, ale také mo¾nost ukládání tìchto pravidel do cache
pamìti.  Konfigurace SSSD pro tento úèel je popsána v~manuálové stránce
...
\subsubsection{Aktualizace SUDO pravidel v~cache pamìti}
...
poèet dotazù na server. SUDO, ve spojení s~LDAP adresáøem, pracuje v¾dy
s~aktuálními pravidly, proto¾e se dotazuje LDAP adresáøe pøi ka¾dém spu¹tìní. SSSD
se dotazuje LDAP serveru pouze v~pøípadì, kdy dané pravidlo v~cache pamìti ji¾
není validní. Aktualizace se poté sna¾í reflektovat stav SUDO pravidel na
serveru v~cache pamìti SSSD.

S ohledem na vytyèené cíle této práce nás ov¹em dále bude SSSD zajímat pouze
z~pohledu programu SUDO. Tedy jakým zpùsobem SSSD sudo pravidla zpracovává na
stranì klienta.
...
programming} paradigma, ale naopak událostmi øízené paradigma\footnote{Event-driven
...
\begin{enumerate} 
	\item èeká na událost, nebo 
	\item provádí obsluhu této události.
\end{enumerate}


Synchronní funkce poskytují programátorovi pouze jedno rozhraní, pøes které je
mo¾no specifikovat vstupní parametry funkce a získat výsledek dané funkce.
Naproti tomu asynchronní funkce, pomocí kterých je mo¾no realizovat vý¹e zmínìné
události, poskytují rozhraní dvì. Jedno pro pøedání parametrù funkci a získání
výsledkù a druhé pro zaslání samotného po¾adavku a uvìdomìní programátora o
dokonèení této asynchronní události.  Funkce, která je zavolána jako výsledek
asynchronní funkce, se nazývá zpìtné volání neboli tzv. callback. Tato rozhraní
zobrazuje obrázek \ref{fig:interfaces}.
...
SSSD pou¾ívá pro vytváøení událostí knihovnu \emph{tevent} \cite{tevent}, která
implementuje asynchronní funkce pomocí tzv. \emph{tevent requests}. Jejich grafické
znázornìní popisuje obrázek \ref{tevent_requests} a s tím je spojena následující konvence:

\begin{itemize}
	\item S~událostí jsou spojena privátní data, která mají pøíponu
		\texttt{\_state}.
...
Pøi vytváøení události je nejprve tøeba vytvoøenou událost spojit s~ní privátní
data. Knihovna tevent pro alokaci dynamické pamìti pou¾ívá rovnì¾ knihovnu
talloc. Asynchronní funkci je mo¾né zavolat pomocí funkce \uv{request\_send},
jméno funkce je mo¾né si zvolit libovolnì, napø. \texttt{ldap\_query\_send}.
Poté probíhá asynchronní provádìní programu. V~okam¾iku kdy bude událost
oznaèena jako dokonèená, napø. pomocí funkce \texttt{tevent\_req\_done()},
nastane zpìtné volání tj. funkce \uv{request\_done}. V~této funkci je mo¾né
zpracovat výsledky dané události a následnì ji odstranit.  
...
data, bylo by nutné tato data pøi ukonèení zanoøené události uvolnit.
Privátními daty ov¹em mù¾e být komplexní struktura, potenciálnì obsahující
ukazatele na dal¹í struktury. Z~tohoto dùvodu pou¾ívá knihovna tevent pro
alokaci pamìti rovnì¾ alokátor talloc, který ve¹kerá data zanoøených
asynchronních funkcí alokuje hierarchicky. Pøi dokonèení první
události, tj. té, která volala dal¹í asynchronní funkce, je
poté najednou uvolnìna ve¹kerá pøidìlená pamì», tj. také ve¹kerá pamì», která
byla pøidìlena v¹em zanoøeným událostem.
...
SSSD se skládá z~nìkolika komponent. Hlavním procesem je zde
...
po¾adavky od klientských aplikací, jako jsou napø. \texttt{sudo}, \texttt{id}
nebo \texttt{getent}, a vracejí odpovìdi na tyto po¾adavky.  Smyslem tìchto
procesù je vrátit, za pomoci cache pamìti, co nejrychleji odpovìï na dotaz,
který jím klientská aplikace zaslala. Proto by mìly provádìt co nejménì
algoritmicky nároèných operací a v~podstatì pouze èíst a pøedávat data z~cache
pamìti SSSD. Komunikují tedy pouze s~cache pamìtí nebo poskytovali dat, ale
nikdy nekomunikují se vzdáleným serverem. V~následujícím textu bude pro ka¾dý
takový proces pou¾íván výraz \textbf{responder}.
...
modul. Napøíklad provider identit mù¾e pou¾ít \emph{ldap plugin} pro pøístup
k~identitám ulo¾ený v~LDAP adresáøi, \emph{ad plugin} pro identity spravované
technologií Active Direcetory. Pro SUDO providera je mo¾né pou¾ít pluginy
...
proces je nazýván \emph{backend}. Backend pou¾ívá providery pro komunikaci
se vzdálenými servery.  

Ka¾dý backend má ve své interní struktuøe ukazatel na pole \texttt{bet\_info},
které obsahuje informace o~providerech, které daný backend pou¾ívá. Ka¾dá
polo¾ka tohoto pole je tvoøena strukturou \texttt{bet\_info}. Ta obsahuje napø.
jméno pluginu, který se má pro daný provider pou¾ít, ukazatel na privátní data
nebo ukazatel na strukturu \texttt{bet\_ops}, kde se nachází ukazatel na funkci,
kterou volají respondeøi, pokud chtìjí danému providerovi zaslat nìjaký
po¾adavek. Jméno této funkce má v~SSSD obecnì pøíponu
\emph{\_handler}
a takto bude oznaèována i v~následujícím textu.
Polo¾ku pole \texttt{bet\_info}, pro SUDO providera pou¾ívající ldap plugin, zobrazuje obrázek
...
upozorní respondera na to, ¾e byla cache pamì» aktualizována.
...
Na klientském poèítaèi je nainstalován démon SSSD. Uva¾ujme, ¾e klient je èlenem
...
proto se rozhodne pou¾ít program SUDO. V~tomto okam¾iku musí SUDO provést
následující tøi úlohy: 
\begin{enumerate}
...
funkce poté volají NSS knihovnu . Ta je dynamicky naèítána aplikacemi , které
potøebují komunikovat s~SSSD.  Získávání informací o~u¾ivatelích je velice èastá
operace, proto tato knihovna disponuje
svojí cache pamìtí, která se oznaèuje jako tzv. \emph{fast cache}. Tato cache
pamìt není souèástí SSSD.  Jesli¾e NSS ve své cache pamìti daného u¾ivatele
nenajde, po¹le dotaz, na tohoto u¾ivatele, NSS responderu, co¾ je jeden
z~procesù SSSD.

NSS responder se nejprve podívá do sysdb, jestli¾e zde daného u¾ivatele nalezne,
vrátí jej zpìt knihovnì NSS. V~opaèném pøípadì po¹le po¾adavek provideru
identit, aby provedl aktualizaci cache pamìti. Ten kontaktuje vzdálený server,
aktualizuje cache pamì» SSSD a upozorní o~tom NSS respondera. Ten poté vrátí
informace o~u¾ivateli NSS knihovnì a ta je pøedá programu SUDO.

\subsection{Autentizace u¾ivatele} 
Jestli¾e má SUDO informace o~u¾ivateli, pak musí provést autentizaci tohoto
u¾ivatele. Autentizaci prování knihovna PAM. Ta nemá 
vlastní cache pamì» a posílá tak po¾adavek pøímo PAM responderu. Ovìøení poté
probíhá podobným zpùsobem jako u~NSS respondera.  PAM responder ov¹em ¾ádá
o~aktualizaci cache pamìti providera, který zaji¹»uje autentizaci\footnote{v
\texttt{sssd.conf} volba \texttt{auth\_provider}}. A~navíc znovu provádí ovìøení
daného u¾ivatele oproti vzdálenému serveru. 

\subsection{Získání sudo pravidel}
Jestli¾e autentizace u¾ivatele probìhla úspì¹nì, pak pøichází na øadu SUDO
responder. Ten nejprve hledá po¾adované pravidlo v~cache pamìti. Jestli¾e se v
cache pamìti nachází a je stále validní, pak nastává tzv. \emph{cache hit}.
V~tomto pøípadì je pravidlo vráceno programu SUDO. 
...
aktualizaci se SUDO responder opìt dotá¾e cache pamìti na po¾adované pravidlo.
...
Jestli¾e, po druhém dotazu do cache pamìti, není po¾adované pravidlo nalezeno, pak
...
SUDO provider provádí ve¹kerou práci pøi komunikaci s~LDAP serverem. Pomocí
volby \texttt{sudo\_provider} v~konfiguraèním souboru \texttt{sssd.conf} je
mo¾no urèit, který modul bute tento provider pou¾ívat. Na výbìr jsou moduly 
\uv{ldap} nebo \uv{ipa}\footnote{\texttt{sudo\_provider=ldap} nebo
\texttt{sudo\_provider=ipa}}.  Tato sekce popisuje funkcionalitu ldap modulu,
který je nutno pou¾ít, máme-li sudo pravidla ulo¾ena na LDAP serveru v~nativním
LDAP SUDO schématu. Tento modul provádí sta¾ení, zpracování a ulo¾ení tìchto
pravidel do sysdb. Skládá se z~nìkolika modulù, které jsou uvedeny na obrázku 
\ref{ldap_provider_high_level} a popsány v následujících sekcích.
...
\subsection {Inicializace ldap modulu}
\label{ldap_sudo_provider_initialization} Inicializaci zaji¹»uje modul
\texttt{sdap\_sudo.c}, který dále pou¾ívá modul
\texttt{sdap\_async\_sudo\_hostinfo.c} pro získání v¹ech jmen a IP adres daného
poèítaèe, která jsou získávána pomocí systémového volání \texttt{gethostname()}.
Jestli¾e je ji¾ jméno poèítaèe specifikováno jako FQDN\footnote{Fully Qualified
	Domain Name}, pak ji¾ dal¹í vyhledávání neprobíhá. V~opaèném pøípadì se tento
	modul pokusí získat FQDN pomocí asynchronního volání
	\texttt{resolv\_gethostbyname\_send()}. I~kdy¾ jsou IP adresy a jméno poèítaèe
	získávána automaticky, je mo¾né je specifikovat pøímo v~\texttt{sssd.conf}
	pomocí voleb \texttt{ldap\_sudo\_hostnames} a \texttt{ldap\_sudo\_ip}.
	Pou¾itím tìchto voleb je mo¾né stahovat i taková SUDO pravidla, která nejsou
	aplikovatelná na klientský poèítaè, co¾ je pou¾íváno pøevá¾nì pro testování.
	Tento modul je na obrázku \ref{ldap_provider_high_level} mo¾né najít, jako blok
	\uv{Get host information}. 

 Modul \texttt{sdap\_sudo.c} dále provádí inicializaci interních struktur LDAP
 SUDO providera a nastavení handleru, tj. inicializaci struktury
 \texttt{bet\_info} jak popisuje sekce \ref{sssd:architecture}. Jsou zde také
 definovaný funkce, které nastavují parametry pro provádìní prùbì¾ných
 aktualizací, které jsou dále popsány v sekci \ref{sec:aktualizace}.
...
aktualizací, které zaji¹tuje modul \texttt{sdap\_async\_sudo\_time.c}, který je
na obrázku \ref{ldap_provider_high_level} zobrazen jako blok \uv{\emph{Schedule
Refresh}}. Jak èasto budou tyto aktualizace provádìny je mo¾né ovlivnit
v~\texttt{sssd.conf} pomocí voleb \texttt{ldap\_sudo\_full\_refresh\_interval}
a \texttt{ldap\_sudo\_smart\_refresh\_interval}.
...
správnì, jeliko¾ by se v~sysdb nenacházela ¾ádná pravidla, za
...
pomocí parametrù, které jsou mu pøedány. Parametry mohou být napøíklad LDAP a
SYSDB filtry, které se mají pou¾ít. Pøedpøipravení parametrù pro konkrétní
aktualizace provádìjí následující funkce: 
\begin{itemize} 
...
Tento typ aktualizace provede smazání v¹ech pravidel z~cache pamìti a sta¾ení
v¹ech pravidel ze serveru. SUDO provider pro ni pou¾ívá LDAP filtr
\ref{ldap:full_refresh} Tento i v¹echny následující filtry se øídí syntaxí
specifikovaou dokumentem RFC2254 \cite{rfc2254}, jsou pouze doplnìny bílými
znaky pro lep¹í èitelnost.

\def\lstlistingname{Obrázek}
\lstset{
	language=bash,
	basicstyle=\normalsize\ttfamily,
...
Filtr \ref{ldap:full_refresh} vyhledá v¹echna SUDO pravidla, která jsou
aplikovatelná na klientský poèítaè. Jméno poèítaèe (\texttt{hostname} a
\texttt{hostname.domain}) a IP adresy jsou získány pøi inicializaci. Filtr také
zachytí v¹echna pravidla aplikovatelná na jakoukoliv sí»ovou skupinu poèítaèù
\footnote{\texttt{(sudoHost=+*)}}, proto¾e nemá informaci o~tom, ve kterých
sí»ových skupinách se poèítaè nachází.

Pøed ulo¾ením sta¾ených pravidel jsou nejprve ze sysdb odstranìny \textbf{v¹echna} SUDO
...
Úplná aktualizace se neprovádí velmi èasto, proto¾e pøi velkém mno¾ství pravidel
generuje velký sí»ový provoz. Provádí se pouze pøi spu¹tìní SSSD, pøi
detekci zmìny pravidel na serveru nebo periodicky s~men¹í frekvencí ne¾ prùbì¾né
aktualizace. Pøi zachování výchozího nastavení se tento typ aktualizace provádní
...
Smyslem prùbì¾ných aktualizací je periodicky stahovat taková pravidla,
která jsou na serveru novì pøidána nebo byla od poslední aktualizace
modifikována.  Pro detekci modifikovaných pravidel se, pøi prùbì¾ných
aktualizacích, vyu¾ívá USN plugin a jeho \texttt{EntryUSN} atribut, viz USN
plugin prezentovaný v~sekci \ref{ipa_sudo_schema}.

Pøedpokládejme pøíklad, kdy je na klientském poèítaèi spu¹tìno SSSD a tento poèítaè má
parametry, které specifikuje tabulka \ref{table:computer}.

\begin{table}[h]
  \begin{center}	
	\caption[Table caption text]{Parametry poèítaèe.}
	\label{table:computer}
  \begin{tabular}{| l | l |} \hline
	Doménové jméno: & \texttt{client1.example.cz} \\ \hline\hline
	IPv4 adresa:    & \texttt{192.168.0.2} \\\hline
	IPv6 adresa: 		& \texttt{fe80::a00:27ff:fe71:1191} \\\hline
  \end{tabular}
  \end{center}
\end{table}

Pøedpokládejme dále, ¾e v sysdb ji¾ máme sta¾ená nìjaká pravidla a nejvy¹¹í
hodnota atributu \texttt{EntryUSN}, která se mezi pravidly nachází, je
\textbf{9270}. SUDO provider by vytvoøil LDAP dotaz, který by pou¾il LDAP filtr
\ref{ldap:smart_refresh}, který by vyhledal v¹echna pravidla, jejich¾
hodnota atributu \texttt{entryUSN} je vìt¹í ne¾ 9270.
...
serveru vìt¹í hodnoty \texttt{entryUSN} atributù, ne¾ nejvìt¹í známá hodnota
v~sysdb.

V~manuálových stránkách a v~kódu SSSD je mo¾né tento typ aktualizace vyhledat
jako \uv{\emph{smart refresh}}.

\subsubsection {Aktualizace pravidel}
\label{ch:sssd:sec:ldap_provider:rules_refresh_implementation}
Tento typ aktualizace je proveden pøi ka¾dém spu¹tìní programu SUDO. Zajistí aby
u¾ivatel nedostal vìt¹í oprávnìní ne¾ je definováno.  Jestli¾e se v~sysdb pøi
spu¹tìní programu SUDO nachází nìjaká pravidla, která ji¾ nejsou
platná\footnote{Èas po kterém se pravidla v~cache pamìti stanou neplatná, je mo¾no
nastavit v~\texttt{sssd.conf} pomocí volby \texttt{entry\_cache\_sudo\_timeout},
viz. manuálová stránka \texttt{sssd.conf}}, pak se provede jejich aktualizace.

Pøedpokládejme napøíklad situaci, kdy aktualizaci provádí opìt poèítaè, jeho¾ parametry
specifikuje tabulka \ref{table:computer} a v~sysdb ji¾ nejsou validní pravidla
\textbf{rule1} a \textbf{rule2}. Nejprve se provede vyhledání tìchto pravidel na
serveru. Pro LDAP dotaz bude vytvoøen filtr, který je zobrazen na obrázku \ref{ldap:rules_refresh}. Jakmile
budou potøebná
pravidla sta¾ena, provede se nejprve odstranìní neplatných pravidel z~cache
pamìti. Pro vyhledání tìchto pravidel v~sysdb pou¾ije SUDO provider filtr
zobrazený na obrázku \ref{sysdb:rules_refresh}. Novì sta¾ená pravidla poté budou
ulo¾eno do sysdb.

\begin{code}[caption={Filtr, který na serveru vyhledá neplatná pravidla z~sysdb.},
         language=Python,label={ldap:rules_refresh}]
(&(&(objectClass=sudoRole)
    (|(cn=rule1)
      (cn=rule2)
    )
...
Aktualizace pravidel, která jsou stále platná, se neprovede a neprovede se ani
aktualizace právì vyvolaného pravidla, za pøedpokladu, ¾e je stále platné.
V~manuálnových stránkách a v~kódu SSSD se tento typ aktualizace oznaèuje
jako \uv{\emph{rules refresh}}. 
...
sysdb. Jeho asynchronní èinnost popisuje obrázek 
\ref{fig:ldap_sudo_provider_tevent_flow}. Modul je vyu¾íván v¹emi, vý¹e
zmínìnými, typy aktualizací. Konkrétní typ aktualizace nejprve nastaví své
parametry, jako napø. LDAP a SYSDB filtry a poté zavolá funkci
\texttt{sdap\_sudo\_refresh\_send()}. V této funkci probìhne kontrola
nastavených parametrù, zda je klient stále pøipojen k síti a jestli¾e
ano, pak probìhne inicializace LDAP spojení. Nové LDAP spojení se vytvoøí pouze
v pøípadì, kdy SSSD je¹tì k danému serveru není pøipojeno. Ve funkci
\texttt{sdap\_sudo\_load\_sudoers\_send()} jsou vytvoøeny parametry pro LDAP
dotaz, jako napø. pole po¾adovaných atributù. Funkce
\texttt{sdap\_sudo\_load\_sudoers\_next\_base()}, pomocí asynchronního
volání SDAP API, posílá LDAP dotaz pro získání záznamù o sudo pravidlech. V
okam¾iku kdy je tato asynchronní událost dokonèena, zavolá se funkce
\texttt{sdap\_sudo\_load\_sudoers\_process()}. Jestli¾e jsou sudo pravidla
ulo¾ena v LDAP adresáøi v nativním LDAP SUDO schématu, pak je mo¾né mít pravidla
rozmístìna ve více kontejnerech. 

Poté je nutné provést vyhledání pravidel ve
v¹ech tìchto kontejnerech, v okam¾iku kdy budou sta¾ena v¹echna pravidla je
zavolána funkce \texttt{sdap\_sudo\_refresh\_load\_done()}, která se postará o
modifikaci sysdb a zahájení transakce, která ulo¾í novì sta¾ená pravidla do
cache pamìti, èím¾ bude ukonèen konkrétní typ aktualizace, který tento modul
pou¾il.
...
Ve¹kerou práci s cache pamìtí zaji¹»uje modul \texttt{sysdb\_sudo.c}. Pøed
samotným ulo¾ením pravidel jsou v~sysdb nejprve vyhledány a následnì odstranìny
v¹echny záznamy, které odpovídají SYSDB filtru pro konkrétní typ aktualizace.
Pøi ukládání SUDO pravidel se ka¾dému pravidlu pøidá atribut
...
\texttt{sssd.conf}\footnote{http://jhrozek.fedorapeople.org/sssd/1.11.5/man/sssd.conf.5.html}}.
Jeho hodnota reprezentuje èas, kdy daný záznam stane neplatným. Zároveò je
získána nejvy¹¹í hodnota USN, která ov¹em není ulo¾ena v~cache pamìti spoleènì
s~pravidly, ale v~kontextu \texttt{id\_ctx->srv\_opts} v~promìnné
\texttt{max\_sudo\_value}.
...
LDAP SUDO provideru je pøes handler, pøi spu¹tìní programu SUDO, pøedán typ
aktualizace, který po nìm SUDO respoder ¾ádá a pravidla, která chce SUDO
responder aktualizovat. SUDO responder mù¾e po¾ádat o~úplnou aktualizaci, nebo
o~aktualizaci pravidel, která ji¾ v~sysdb nejsou nadále platná. Tuto situaci
zobrazuje obrázek \ref{ldap_sudo_handler}.
...
modul\footnote{\texttt{sudo\_provider=ipa}}, jestli¾e jsou SUDO pravidla
ulo¾ena na IPA serveru.  Problém ov¹em je, ¾e SSSD nedisponuje IPA modulem pro
SUDO providera, který by podporoval IPA SUDO schéma. Souèasná implementace
tohoto modulu je pouze obálka pro ldap modul. Tento stav je zachycen na obrázku
\ref{ipa_sudo_provider_wrapper}. Samotný LDAP modul rovnì¾ nepodporuje IPA SUDO
schéma a neumí tedy SUDO pravidla v~tomto formátu zpracovat. LDAP modul umí
zpracovat pouze pravidla, která odpovídají nativnímu LDAP SUDO schématu. 

\begin{figure}[H]
	\centering
	\includegraphics{fig/sssd/ipa_sudo_provider/ipa_sudo_wrapper.pdf}
	\caption{Souèasná implementace IPA SUDO modulu.}
	\label{ipa_sudo_provider_wrapper}
\end{figure}

Pro podporu SUDO pravidel v~IPA SUDO formátu vyu¾ívá souèasná implementace SSSD
tzv. \emph{compat plugin}. Tento plugin bì¾í na serveru FreeIPA a zaji¹»uje
pøeklad SUDO pravidel z~IPA SUDO schématu do LDAP SUDO schématu. Funkcionalita
compat pluginu je zobrazena na obrázku \ref{sudo_rules_compat_plugin}. Pøeklad
samotný probíhá pøi ka¾dém vytvoøení nebo modifikaci SUDO pravidla. Takto
...
která jsou aplikovatelná pro daného klienta. Cílem této bakaláøské práce je
právì odstranìní této re¾ie a implementace IPA SUDO pluginu, který bude
zaji¹»ovat podporu nativního IPA SUDO schématu. Návrhem nativního IPA SUDO
providera se zabýva kapitola \ref{ch:native_ipa_sudo_provider}.

\section{Dereferenèní dotaz}\label{sec:deref}
LDAP dotaz umo¾òuje získat záznam, popø. mno¾inu záznamù z LDAP adreáøe.
Jestli¾e se nìjaký atribut, z~tìchto záznamù, odkazuje na jiné záznamy (hodnota
nìkterého z~atributù obsahuje DN jiného záznamu v~adresáøi), Pak pro získání
tìchto záznamù je nutné vytvoøit nový dotaz. Tato situace je zobrazena na
obrázku \ref{fig:sdap_deref}.

\begin{figure}[h]
	\centering
	\includegraphics[scale=1.00]{fig/deref_example.pdf}
	\caption{Pøíklad záznamu, jeho¾ atributy se odkazují na jiný záznam.}
	\label{fig:sdap_deref}
\end{figure}

Chování API knihovny OpenLDAP je mo¾né mìnit pomocí tzv. \uv{\emph{controls}}.
Chování funkce \texttt{ldap\_search\_ext()} je mo¾né modifikovat takovým
zpùsobem, ¾e bude mo¾né získat nejenom po¾adované záznamy, ale také atributy ze
záznamù, na které se daný záznam odkazuje. SDAP rozhraní vyu¾ívá, a pro tento
úèel této, nabízí programátorovi asynchronní funkci
\texttt{sdap\_deref\_search\_send()}.  Server ov¹em musí tento typ \uv{controls}
podporovat\footnote{supportedControl: 1.3.6.1.4.1.4203.666.5.16}. U~Active
Directory se tato technika oznaèuje jako \emph{Attribute Scope
Queries}.\footnote{http://msdn.microsoft.com/en-us/library/aa746418\%28v=vs.85\%29.aspx}
...
Nìkteré specifické detaily byly diskutovány také s~vývojáøi projektu FreeIPA. 
...
pøistoupit zpùsoby popsanými ní¾e.

\subsubsection{Klient jako èlen IPA domény}
První mo¾ností je vyu¾ít záznam, obsahující informace o èlenu IPA domény.  Ka¾dý
èlen má na IPA serveru záznam v~kontejneru
\texttt{cn=computers,cn=accounts,\$DC}. Ka¾dý takový záznam obsahuje napøíklad
atribut \texttt{memberOf}, kde se nacházejí odkazy na záznamy skupin poèítaèù,
sí»ových skupin a SUDO nebo HBAC\footnote{Host-Based Access Control} pravidel,
kterými je daný klient èlenem. Bylo by tedy mo¾né jedním dotazem získat odkazy
na SUDO pravidla, která jsou aplikovatelná na daný klientský poèítaè a dal¹ími
dotazy u¾ konkrétní záznamy SUDO pravidel.  Jestli¾e bychom chtìli získat
DNs\footnote{Distinguished Names} SUDO pravidel aplikovatelných na klientský
poèítaè, jeho¾ doménové jméno je \texttt{client.example.cz}.  Pøíslu¹ný LDAP
dotaz by mohl mít následující parametry:

%\begin{table}[H]
%	\begin{tabular}{ll}
\begin{itemize}
	\item Prohledávána oblast: \texttt{fqdn=client.example.cz,cn=computers,cn=accounts,\$DC},
	\item Filtr: \texttt{"(memberOf=ipauniqueid=*cn=sudorules,cn=sudo,\$DC)"},
	\item Dotazované atributy: \texttt{memberOf}.
\end{itemize}
	%\end{tabular}
%\end{table}
...
atribut a pravidla by tedy bylo nutné dodateènì stáhnout. Druhým problémem
...
bylo nutné získat pomocí dereference odkazù, která je blí¾e
popsána v~sekci \ref{sec:deref}, na dané
záznamy skupiny poèítaèù.

\subsubsection{Specifikace klientského poèítaèe}
\label{design:rules_aplicable_to_host}
Druhou mo¾ností, jak získat kompletní SUDO pravidla, je vyu¾ít stejný pøístup,
jako pou¾ívá souèasná implementace LDAP SUDO providera. Pro výbìr SUDO pravidla
aplikovatelného na konkrétní poèítaè v~IPA doménì jsou v tomto pøípadì pou¾ity
tøi atributy: 
\begin{itemize}
	\item memberHost,
	\item externalHost,
...
\texttt{memberHost} bude v¾dy obsahovat plnì specifikované doménové jméno u
poèítaèe nebo název skupiny poèítaèù. Jestli¾e je pravidlo aplikovatelné na
libovolný poèítaè, pak bude mít atribut \texttt{hostCategory} hodnotu
\texttt{all} a jiné atributy specifikující poèítaè se ji¾ v~záznamu pravidla
neobjeví. Parametry LDAP dotazu, pomocí kterého bychom získali záznamy v¹ech
pravidel aplikovatelných pro klientský poèítaè s~doménovým jménem
\texttt{client.example.cz}, mohou vypadat následovnì:

%\begin{table}[H]
%	\begin{tabular}{ll}
\begin{itemize}
	\item Prohledávána oblast: \texttt{cn=sudorules,cn=sudo,dc=example,dc=cz},
	\item Filtr: \texttt{"($|$(memberHost=client.example.cz)(hostCategory=all))"},
	\item Dotazované atributy: \texttt{memberOf}.
\end{itemize}
	%\end{tabular}
%\end{table}
...
pouze na IPA serveru. Tento problém by bylo mo¾né odstranit dvìmi zpùsoby:
...
Pro implementaci byl vybrán právì druhý uvedený pøístup. Detaily jsou popsány
v~úvodu sekce \ref{sec:implementation}.
...
\texttt{ou=SUDOers,\$DC}. Prohledávanou oblast je mo¾né specifikovat
v~\texttt{sssd.conf} pomocí volby \texttt{ldap\_sudo\_search\_base}.
SUDO pravidla na IPA serveru jsou ov¹em rozmístìna ve tøech kontejnerech, viz
Sekce \ref{ipa_sudo_schema}. Pro sta¾ení kompletních SUDO pravidel se nabízí
nìkolik následujících pøístupù.

\subsubsection{1. Postupné dotazování}
První pøístup, který se nabízí a který je popsán na obrázku 
\ref{rules_downloading:1st_approach}, je sta¾ení v¹ech pravidel, aplikovatelných
na daný poèítaè, z~kontejneru \texttt{cn=sudorules,cn=sudo,\$DC}. Pro tyto
záznamy je ov¹em nezbytné je¹tì stáhnout pøíkazy. Bylo by tedy nutné iterovat
pøes sta¾ené záznamy a pro ka¾dý odkaz na SUDO pøíkaz provést LDAP dotaz, jeho¾
výsledkem by ji¾ byl konkrétní pøíkaz. Pokud by se jednalo o~skupinu pøíkazu,
pak by bylo nutné provést dereferenèní dotaz, jeho¾ výsledkem by byl záznam, kde
by se nacházel ji¾ konkrétní pøíkaz.
...
Pøi rozsáhlé databázi SUDO pravidel by ov¹em dotazování na v¹echny pøíkazy mohlo
znamenat velké mno¾ství LDAP dotazù. Pravidla by navíc mezi tìmito dotazy mohla
být modifikována.
...
aplikovatelných na klientský poèítaè. Tyto sta¾ené záznamy poté projít a
sestavit filtr pro LDAP dotaz, který stáhne v¹echny potøebné pøíkazy pro sta¾ená
pravidla. Pøíklad vytvoøení takového filtru ukazuje obrázek
\ref{fig:ldap_filter_for_cmds}.

\begin{figure}[H]
	\centering
	\includegraphics[scale=1.10]{fig/sssd/new_design/sudo_cmds_filter.pdf}
	\caption{Sestavení LDAP filtru pro SUDO pøíkazy.}
	\label{fig:ldap_filter_for_cmds}
\end{figure}
...
Výhodnou tohoto pøístupu je, ¾e redukuje poèet LDAP dotazù nutných pro sta¾ení
kompletních SUDO pravidel a zároveò nestahuje ¾ádné pøebyteèné informace. Ve
výsledku by tak staèil jeden dotaz pro získání SUDO pravidel a druhý pro sta¾ení
pøíkazù, které jsou potøebné pro tato pravidla.
...
Výhodou tohoto pøístupu je, ¾e pro sta¾ení kompletních SUDO pravidel postaèí
jediný LDAP dotaz. Jejich následný pøeklad je mo¾né provést v~jednom cyklu. Mù¾e
ov¹em nastat situace, kdy se na klienta bude aplikovat napø. pouze jedno SUDO
pravidlo. Jestli¾e se na serveru nachází velké mno¾ství SUDO pravidel a jejich
...
Pøístup, který byl vybrán pro implementaci a dùvody tohoto výbìru, jsou dále
popsány v sekci \ref{sec:implementation}.
...
\ref{sudo_rules_compat_plugin}, pøeklad \emph{v¹ech} sudo pravidel v~souèasné
dobì realizuje compat plugin a je provádìn \textbf{na serveru} FreeIPA.
S~podporovou nativního IPA SUDO schématu se tento pøeklad pøesune na stranu SSSD a
...
odvodit. Atribut \texttt{memberUser}, který na IPA serveru specifikuje u¾ivatele
nebo skupinu, by napø. mohl mít hodnotu: \texttt{memberUser:
cn=students,cn=groups,cn=accounts,\$DC}.  Z této hodnoty je mo¾né odvodit, ¾e je
pravidlo aplikovatelné na v¹echny u¾ivatele skupiny \uv{students}.
U~\texttt{*Category} atribtù je nutné provést pouze zámìnu malých písmen za
velká. 
...
\ref{table:ipa_sudo_attrs_types}. Mohlo by se zdát, ¾e typy \texttt{USER\_GROUP}
a \texttt{HOST\_GROUP} není nutné rozli¹ovat, ov¹em li¹í se v~prefixu, který je
k~nové hodnotì nutno pøidat: Viz pøíklady hodnot atributù \texttt{sudoUser}
...
\texttt{ipaSudoRunAsGroup} pøiøadit libovolnou skupinu. Tento atribut ov¹em mù¾e
ohsahovat odkaz pouze na POSIX skupinu.  Pro tuto chybu byl vytvoøen ticket
èíslo 4314\footnote{https://fedorahosted.org/freeipa/ticket/4314\#no1}.
...
doplníme potøebné pøíkazy. Tento postup je zachycen na obrázku
\ref{fig:axport_alg}.
...
pravidla ve kterých se pøíkaz vyskytl. Jestli¾e $N$ je poèet odkazù na pøíkazy, 
ve sta¾ených záznamech sudo pravidel, pak je nutné $N$-krát iterovat mno¾inou
pøíkazù, aby bylo mo¾né sestavit kompletní sudo pravidla. Tyto problémy je mo¾né
odstranit za pomoci pou¾ití ha¹ovací tabulky.  Tento pøístup zobrazuje schéma
uvedené na obrázku 
\ref{hash_table}.  Jako klíè mù¾e poslou¾it DN záznamu SUDO pøíkazu nebo skupiny
pøíkazù. Hodnotou polo¾ky této tabulky poté mù¾e být seznam v¹ech pravidel, ve
kterých se daný pøíkaz nebo skupina pøíkazù vyskytuje. Je ov¹em nutné si také
uchovat informaci o~tom, zda má být daný pøíkaz povolen/zakázán a také poøadí
daných pøíkazù, na kterém zále¾í, viz Sekce
\ref{sec:sudoers_ordering}.
...
Hlavní výhodou tohoto pøístupu je sní¾ení poètu iterací mno¾inou záznamù pøíkazù. 
Tento pøístup je
...
\subsection{Vyu¾itelnost zasuvného modulu LDAP}
...
realizovatelný\footnote{První prototyp, ipa modulu pro SUDO providera, byl na
této my¹lence zalo¾en.}, ov¹em neodpovídá návrhu SSSD. Kód zaji¹tující pøeklad
pravidel by byl souèástí dynamické knihovny \texttt{libsss\_ipa.so} zatímco kód
LDAP SUDO providera  se nachází v~knihovnì \texttt{libsss\_ldap.so}. SSSD je
ov¹em mo¾no nainstalovat a pou¾ívat bez podpory IPA. Napøíklad je mo¾no vyu¾ít
SSSD pouze ke cachování SUDO pravidel ulo¾ených na LDAP serveru, kde se
samozøejmì nacházejí v~nativním LDAP SUDO schématu. V~takovém pøípadì není
potøeba SUDO pravidla pøekládat a SSSD je nakonfigurováno tak, aby vyu¾ilo LDAP
SUDO providera z~knihovny \texttt{libsss\_ldap.so}. Knihovna
\texttt{libsss\_ipa.so} tedy není potøeba.  Pøi naèítání
\texttt{libsss\_ldap.so} by poté nastal problém, jeliko¾ by se odkazovala na
funkce, které se nacházejí v~knihovnì, která není a ani by nebyla naètena do
pamìti. Proto není mo¾né z~LDAP SUDO providera volat kód z~IPA SUDO providera.
Je ov¹em mo¾né z~IPA SUDO providera volat kód LDAP SUDO providera, LDAP provider
je mo¾né si pøedstavit jako jakýsi stavební blok, nad kterým mohou stavìt
ostatní provideøi, a toho je mo¾no vyu¾ít. Vìt¹í zásah do LDAP SUDO providera by
také mohl vnést mnoho chyb do ji¾ fungujícího a odladìného kódu. 
...
asynchronní události pro sta¾ení pravidel v~IPA SUDO schématu. Pro tato pravidla
...
dotazem do kontejneru \texttt{cn=sudorules,cn=sudo,\$DC}.  Filtr pro LDAP dotaz
specifikuje klienta pomocí FQDN daného poèítaèe a skupin, ve kterých je èlenem.
Pro získání tìchto skupin je pou¾ita funkce \texttt{ipa\_host\_info\_send()}.
Základní filtr, který je pou¾it pro úplnou aktualizaci vypadá následovnì:
...
respondera, která by nepøinesla ¾ádnou výhodu. Hodnoty atributù
\texttt{cn}, tj. jména SUDO pravidel, jsou jedineèná i v~IPA SUDO schématu, není
tedy potøeba tuto zmìnu provádìt. Pøi prùbì¾ných aktualizacích je, pro dekekci
novì pøidaných nebo modifikovaných pravidel, opìt vyu¾it operaèní atribut
\texttt{EntryUSN}.

Sta¾ení kompletních SUDO pravidel, tj. záznamy pravidel a pøíkazù, je provedeno
pomocí dvou dotazù pro v¹echny typy aktualizací. Pro sta¾ení pravidel jsou tedy
zapotøebí \textbf{nejvý¹e tøi} LDAP dotazy a dva dotazy postaèí v pøípadì, kdy
sta¾ená pravidla neobsahují odkazy na dal¹í záznamy, tj. odkazy na záznamy
pøíkazù.

Zva¾ována byla také varianta, kde by se pro úplnou aktualizaci postupovalo podle
metody ze sekce \ref{all_at_once}, tzn.  sta¾ení SUDO pravidel a zároveò v¹ech
pøíkazù v~jediném LDAP dotazu. SSSD pou¾ívá své SDAP API pro provádìní LDAP
dotazù. Výsledky LDAP a SDAP dotazù ov¹em v~nìkterých pøípadech nejsou
ekvivalentní.
Záznamy, pøijaté ze serveru, jsou filtrovány pøedtím, ne¾ jsou pøedány
programátorovi. To zaji¹»uje funkce \texttt{sdap\_parse\_enrty()}. Ta vrací pouze
záznamy, které spadají do jedné konkrétní tøídy.  Jestli¾e pøijaté záznamy
nejsou odvozeny z~této konkrétní tøídy, pak jsou zahozeny. 

Záznamy o~sudo pravidlech a pøíkazy, pro tyto pravidla, jsou ulo¾eny v~separátních
kontejnerech a jsou definovány pomocí jiných tøíd, viz Tabulka
\ref{table:ipa_rules_containers}. Z~tohoto dùvodu není mo¾né jedním SDAP dotazem
získat záznamy sudo pravidel a zárovneò záznamy pøíkazù pro tyto pravidla.
...
SUDO prvoder, pak sta¾ená pravidla je¹tì nemohou být ulo¾ena do sysdb a proto je
událost sta¾ení pravidel oznaèena jako dokonèena pøed prací s~cache pamìtí.
Z~tohoto modulu jsou dále vyu¾ity funkce, které zaji¹»ují práci s~cache pamìtí,
tzn. odstraòování nevalidních a ukládání novì sta¾ených a ji¾
\textbf{pøelo¾ených} pravidel. Pro plánování pravidelných aktualizací byl pou¾it
interní plánovaè periodických událostí \emph{ptask}. 

Novì navr¾ený provider se skládá z~následujících modulù:

\begin{itemize}
	\item \texttt{ipa\_SUDO.c}:	provádí poèáteèní inicializaci IPA SUDO
		providera, nastavuje SUDO handler a provádí nastavení prùbì¾ných
		aktualizací.
	...
	\item \texttt{ipa\_async\_sudo.c}: asynchronní sta¾ení SUDO pravidel z~IPA
		serveru.
...
serverem. Jednotkové testy je mo¾né provádìt bez pøítomnosti IPA serveru èi
pøipojení k~síti. Mock objekty tedy simulují IPA server a SUDO pravidla, která
se na nìm nacházejí.

\subsection{Metodika}
Reálná SUDO pravidla si ka¾dá spoleènost peèlivì støe¾í a proto nebylo mo¾né
taková pravidla získat. Pro testování byla vytvoøena sada pravidel, která
pokrývá následující pøípady pou¾ití:
...
% 5. Závìr
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%absolutne tady chybi zhodnoceni funkcnosti vysledneho dila 
% - je to funcni
% -bylo to testovano i jinak nez pres uvedena pravidla
% - bylo to predano redhat? bude to pouzito?
\chapter{Závìr}
Tato bakaláøská práce popisuje LDAP schémata, která pou¾ívají program SUDO a
server FreeIPA k~ukládání SUDO pravidel v~LDAP adresáøích. Srovnává jejich
rozdíly a výhody jejich pou¾ití. Dále dokumentuje funkcionalitu ldap modulu,
který má k~dispozici SUDO provider. Tato dokumentace odkrývá nedostatek démonu
...
schéma podporuje a pøená¹í tak pøeklad SUDO pravidel ze serveru FreeIPA na
klientský démon SSSD. Pøi návrhu byla rovnì¾ nalezena jedna chyba ve webovém
rozhraní FreeIPA. Nový modul pro SUDO providera je plnì funkèní a bude zaøazen
do dal¹í beta verze SSSD. 

Pøed samotným návrhem a implementací bylo nutné nastudovat koncepty a pou¾ití
mnoha technologií, napø. knihoven talloc, tevent, ldb, PAM nebo NSS. Dále bylo
nutné se seznámit s konfigurací a správou serveru FreeIPA, démonu SSSD a
programu SUDO.

V~práci je dále mo¾no pokraèovat napøíklad roz¹íøením jednotkových testù. Mohly
by také být provedeny výkonnostní testy, na jejich¾ základì by mohla být
stanovena hranice pro výbìr konkrétního algoritmu pro pøeklad pravidel. Pro
plánování aktualizací u~LDAP SUDO provideru by takté¾ mohl být pou¾it ptask
plánovaè, popø. by toto rozhraní mohlo být navr¾eno takovým zpùsobem, aby jej
mohly vyu¾ít oba moduly pro SUDO providera. Integrace obou modulù SUDO providera
by mohla být provedena je¹tì lepé, kdyby v¹echny spoleèné èásti vyu¾ívaly stejný
kód, èím¾ by se docílilo nulové redundance kódu. To by ov¹em vy¾adovalo nový
návrh celého SUDO providera, co¾ by znamenalo kompletní pøepsání jak ldap tak
ipa modulù pro SUDO providera.
