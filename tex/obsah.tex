...
této práce.
V~kapitole \ref{politics_managament} jsou popsány problémy, které vznikají
...
který je nezbytný pro vyu¾ití tohoto schématu.
...
Je mo¾né napøíklad vyu¾tí NIS\footnote{Network Information Service} nebo
...
DN\cite[Kapitola 1]{deref}. Jestli¾e se tedy atribut \texttt{user} odkazuje na
záznam, s informacemi o u¾ivateli, bude hodnota tohoto atributu tvoøena právì DN
daného záznamu:
...
IPA SUDO schéma definuje novou tøídu pro SUDO pravidla, jejich¾ definice vypadá
následovnì:
...
cn=users,cn=accounts,\$DC\footnote{Domain Controller}}. Pomocí \texttt{externalUser} atributu je také mo¾né
...
centralizovanì napøíklad pomocí technologií LDAP+Kerberos, musí provést
nastavení NSS\footnote{Name Service Switch} a PAM\footnote{Pluggable
Authentication Modules}. Musí pou¾ít správnì moduly pro tuto podporu. Napøíklad
\texttt{nss\_pam\_ldapd} pro pøístup k identitám a autentizacím na LDAP serveru.
Nebo \texttt{pam\_krb5} pro Kerberos autentizaci. Správná konfigurace
mù¾e být pro bì¾ného u¾ivatele velice slo¾itá. Ov¹em i po správné konfiguraci
zde zùstávají dùle¾ité nedostatky tohoto øe¹ení:

\begin{enumerate}
	\item Jakmile je u¾ivatelský poèítaè odpojen od sítì, pak se u¾ivatel nemù¾e
		pøihlásit. U¾ivatel to pak vìt¹inou øe¹í separátním u¾ivatelským úètem.
...
které spravují identity a jejich autentizaci a politiky. Identity a jejich
autentizace tak mù¾e být spravována pomocí technologií LDAP+Kerberos, FreeIPA
nebo Active Directory. Je také mo¾né jej pou¾ít pouze pro pøístup k sudo
pravidlùm spravovaných centrálnì pomocí LDAP adresáøe.

Umo¾òuje u¾ivateli pøipojení do více domén zároveò. Pokud napøíklad klient
støídá prostøední mezi prací a ¹kolou, nemusí stále mìnit slo¾itou konfiguraci.
SSSD pøiná¹í také offline podporu a cachování. Informace o identitách nebo napø.
sudo pravidla jsou ukládána do cache pamìti, která se nachází na disku klienta.
Tyto informace je tedy mo¾né vyu¾ívat v situacích kdy se klient ocitne bez
pøipojení k síti. To v¹e je mo¾né nakonfigurovat pomocí jediného konfiguraèního
souboru \texttt{/etc/sssd/sssd.conf}. 
...
Pro cíle této bakaláøské práce nás ov¹em 
dále bude SSSD zajímat pouze z~pohledu programu SUDO. Tedy jakým zpùsobem SSSD
sudo pravidla zpracovává.
...
\uv{ldap}\footnote{\texttt{sudo\_provider=ldap}} a
\uv{ipa}\footnote{\texttt{sudo\_provider=ipa}}.  Tato sekce popisuje
funkcionalitu ldap pluginu, který je nutno pou¾ít, máme-li sudo pravidla ulo¾ena
na LDAP serveru v nativním LDAP SUDO schématu. Tento plugin provádí sta¾ení,
zpracování a ulo¾ení tìchto pravidel do sysdb. Skládá se z~nìkolika modulù
jejich¾ celkovou funkcionalitu popisuje schéma
...
Úplná aktualizace se neprovádí velmi èasto, proto¾e pøi velkém mno¾ství
...
která jsou na serveru novì pøidány nebo byly od poslední aktualizace
modifikovány.  Pro detekci modifikovaných pravidel se pøi prùbì¾ných
...
Tato aktualizace je provedena pøi ka¾dém spu¹tìní programu SUDO. Zajistí aby
...
platná\footnote{Èas po kterém pravidla v~cache pamìti stanou neplatná je mo¾no
nastavit v~\texttt{sssd.conf} pomocí volby \texttt{entry\_cache\_sudo\_timeout},
viz.  \texttt{manuálová stránka
sssd.conf\footnote{http://jhrozek.fedorapeople.org/sssd/1.11.5/man/sssd.conf.5.html}}},
pak se provede jejich aktualizace. Pøedpokládejme pøíklad, kdy v~sysdb ji¾
nejsou validní pravidla \textbf{rule1} a \textbf{rule2}. Nejprve se provede
vyhledání tìchto pravidel na serveru. Pro LDAP dotaz bude pou¾it filtr
\ref{ldap:rules_refresh}. Jakmile budou potøebná pravidla sta¾ena, provede se
nejprve odstranìní neplatných pravidel z cache pamìti. Pro vyhledání tìchto
