...
\begin{itemize}
	...
	\item vedet co to je sudo a sektat se sním alespoò na u¾ivatelské úrovní
\end{itemize}
...
\begin{itemize}
	\item názvy souborù, klíèových slov jazyka C, jmen identifikátorù je pou¾it
		texttt
	\item hodnoty atributù \texttt{ipaUniqueID} byly zámìrnì zkracovány, kvùli lep¹í èitelnosti obrázkù a tabulek.
\end{itemize}
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\chapter{Správa bezpeènostních politik identit} Tato kapitola popisuje zpùsob
vyu¾ití utility sudo a serveru FreeIPA ve spojení s démonem SSSD pro správu
bezpeènostních politik identit v linuxových prostøedích. Zabývá se mo¾nými
zpùsoby distribuce tìchto politik, kterými se sudo øídí. Popisuje a
srovnává LDAP schémata, která definují sudo a FreeIPA, pro uchování sudo
pravidel v LDAP adresáøi. Tento zpùsob správy bezpeènostních politik identit je
pou¾íván v Redhat Enterprise prostøedích.
...
pøípadù je jiným u¾ivatelem právì správce linuxového systému, tj. u¾ivatel
\emph{root}. Tedy u¾ivatel, který má nejvy¹¹í oprávnìní. Pokud u¾ivatel vyvolá
nìjaký pøíkaz za pomocí programu sudo, pak není vyzván k zadání hesla správce
systemu, ale k zadání hesla daného u¾ivatele.\cite[Sekce
4.4.3.2]{Security_Guide} Bezpeènostní politiky, kterými se sudo øídí se nazývají
sudo pravidla. Nìkdy také oznaèovaná jako \emph{sudoers}. Sudo pravidla se
...
\begin{enumerate}
	...
	\item \texttt{command\_list} \,--\, seznam povolených/zakázaných pøíkazù pøíkazù 
\end{enumerate}
...
stanici. Ka¾dou stanici mù¾e pou¾ít více ne¾ jeden u¾ivatel, který mù¾e potøebovat provést
úlohu, pro kterou potøebuje vy¹¹í oprávnìní. Proto by chtìl správce systému
distribuovat sudo pravidla mezi více poèítaèù. Sudo ov¹em nemá nativní zpùsob jak
...
{\color{red}{
TODO: zdroj nemá nativní podporu pro ¹íøení ...
}}
...
LDAP adresáøe mohou uchovávat rùzné typy informací a jsou tak pou¾ívány
organizacemi a institucemi k ukládání informací o jejich zamìstnancích,
u¾ivatelích nebo zaøízeních jejich poèítaèových sítí. LDAP adresáøe jsou
...
budou uchovávat informace, které nebudou pøíli¹ èasto modifikovány.\cite{}. Sudo
pravidla jsou vìt¹inou jednou definována správcem systému a mnohokrát pou¾ita
u¾ivateli. Ulo¾ení sudo pravidel do LDAP adresáøe je tedy velice výhodné a
...
\begin{enumerate}
	...
	\item Jestli-¾e udìlá správce v sudoers souboru chybu\footnote{Je mo¾né tomu
...
\subsection{Nativní LDAP sudo schéma} Sudo má vlastní podporu pro sudo pravidla
ulo¾ená v LDAP adresáøi. Definuje také schéma, kterým je nutno se øídit. Tøída
\texttt{sudoRole}, která definuje sudo pravidlo vypadá následovnì:

\lstinputlisting{listings/sudoRole}

Atribut \texttt{sudoUser} specifikuje u¾ivatele na kterého bude pravidlo
aplikováno. U¾ivatele je mo¾no specifikovat nìkolika zpùsoby, které zobrazuje
...
\begin{table}[h]
  ...
  \begin{tabular}{| l | l |} \hline
    ...
    u¾ivatelské jméno & xsruba03 \\ \hline
    uid\footnote{User ID} & \#9843 \\ \hline
    skupina u¾ivatelù & \%3bit \\ \hline
    gid\footnote{Group ID} & \%\#43 \\ \hline
    unixová sítová skupina & +admins \\ \hline
    non-unixová sítová skupina & \%:finance\_dp \\ \hline
    v¹ichni u¾ivatelé & ALL \\ \hline
  \end{tabular}
  \end{center}
	\caption[Table caption text]{Tabulka popisující mo¾né hodnoty atributu sudoUser.}
	\label{table:sudoUser}
\end{table}

Jestli¾e víme na koho bude pravidlo aplikováno, musíme dále specifikovat na
kterých poèítaèích bude toho pravidlo platné. Toho je mo¾no docílit pomocí
atributu \texttt{sudoHost}, který popisuje tabulka \ref{table:sudoHost}.

\begin{table}[h]
  \begin{center}	
  \begin{tabular}{| l | l |} \hline
    \textbf{Mo¾ná hodnota atributu} & \textbf{Popis} \\ \hline\hline
    jméno poèítaèe & client.example.cz \\ \hline
    IP adresa & fe80::8a9f:faff:fe0c:b989 \\ \hline
    adresa sítì & 192.168.1.0/24 \\ \hline
    sítová skupina & +servers \\ \hline
    kterýkoliv poèítaè & ALL \\ \hline
  \end{tabular}
  \end{center}
	\caption[Table caption text]{Tabulka popisující mo¾né hodnoty atributu sudoHost.}
...
také mo¾né specifikovat parametry daného pøíkazu.

\begin{table}[h]
  \begin{center}	
  \begin{tabular}{| l | l |} \hline
    \textbf{Mo¾ná hodnota atributu} & \textbf{Popis} \\ \hline\hline
    povolený pøíkazzakázaný pøíkaz & blkid /dev/sda1 \\ \hline
    zakázaný pøíkaz & !blkid \\ \hline
    v¹echny pøíkazy & ALL \\ \hline
  \end{tabular}
  \end{center}
...
byl také pou¾íván atribut \texttt{sudoRunAs}. Ten u¾  se dnes ov¹em nedoporuèuje
pou¾ívat.

K sudo pravidlu je také mo¾né specifikovat rùzné volby, které napøíklad urèují
jak bude vypadat øetìzec, který po¾ádá u¾ivatele o zadaní hesla. Specifikovat
zda bude heslo vùbec po¾adováno a mnoho dal¹ích. V¹echny toto volby je mo¾né
specifikovat pomocí atributu \texttt{sudoOption}\cite[Sekce SUDOERS
...
FreeIPA server øe¹í expiraci pravidel jiným zpùsobem, viz. USN plugin v sekce
\ref{ipa_sudo_schema}, nebudeme se tìmito atributy dále zabývat.

Posledním pou¾itelným atributem je \texttt{sudoOrder}. Tento atribut pøedstavuje
èíselnou hodnotu,  která je pou¾ita v situaci, kdy bude mo¾no aplikovat více ne¾
jedno pravidlo. V¾dy bude vybrán záznam s vìt¹í hodnotou. To reflektuje chování
\uv{poslední schody} souboru \texttt{/etc/sudoers}. Pøedpokládejme následující
dvì pravidla:
...
\texttt{blkid}. Jestli¾e ov¹em prohodíme tyto pravidla, pak u¾ivatel bude mít
oprávnìní tento pøíkaz spustit. Poøadí tedy výraznì ovlivòuje chování.  LDAP
protokol ov¹em negarantuje poøadí ve kterém jsou pøíjaty jednotlivé záznamy, tj.
sudo pravidla. Proto bylo nutné definovat atribut pomocí kterého je mo¾né øadit
jednotlivá pravidla.

{\color{red}{TODO odkaz do RFC.}}

Záznam ekvivalentního suda pravidla \ref{sudoer} by poté v LDAP adresáøi vypadal tak, jak jej
popisuje obrázek \ref{fig:ldap_sudoer}.
...
Detailní popis tohoto schématu je mo¾né nalézt v manuálových stránkách
texttt{sudoers.ldap} nebo
online\footnote{http://www.sudo.ws/sudoers.ldap.man.html}.

\subsection{Integrace identit a politi}
Vìt¹ina spoleèností dnes chce spravovat své identity centrálnì.
LDAP adresáøe jsou pro tento úèel tedy velice vyu¾ívány. Napøíklad ve Windows
prostøedích je dnes roz¹íøeno øe¹ení Active Directory nebo \emph{389 Directory
server} v linuxových prostøedích. Na tento server je tedy mo¾né nativní sudo
schéma nahrát a zaèít tak sudo pravidla spravovat centrálnì.  Pøístup ke správì
identit a jejich bezpeènostních polit pomocí LDAP adresáøe a nativního sudo
schématu má ov¹em nìkolik nevýhod: nativní ldap sudo schéma je pevnì dané a není
mo¾né jej mìnit, záznamy jiné tøídy sudo neumí zpracovat pravidla je mo¾no
definovat pouze pomocí pøíkazového øádku, popø. pomocí souborù v
LDIF\footnote{LDAP Data Interchange Format} formátu v pøípadì, ¾e se u¾ivatel
ocitne bez sítì, pak není mo¾né sudo pou¾ít (pøípadnì lze pracovat pouze s
lokálními pravidly definovanými v souboru \texttt{/etc/sudoers} pøi ¹patné
konfiguraci NSS\footnote{Name Service Switch} mù¾e dojít k pøekrytí u¾ivatele z
LDAP adresáøe s lokálním u¾ivatelem
...
rozhodne spravovat jak identity jak bezpeènostní politiky tìchto identit
centrálnì, pak by chtìl aby tyto entity byly vzájemnì propojeny.  Toho ov¹em
není mo¾né dosáhnout s pou¾itím nativního LDAP sudo schématu, jeliko¾ musí být
specifikovány jako znakové øetìzce. To nedovoluje ¾ádnou integraci sudo pravidel
s ji¾ existujícími identitami. Tento problém se sna¾í øe¹it projekt FreeIPA,
kterým se zabývá následující sekce.
...
Sekce \ref{ldap_sudo_scheme_problems} popisovala problémy, které vznikají pøi
pou¾ití nativního LDAP sudo schématu z pohledu domény. Toto schéma sice mù¾e být
nuhráno na IPA server, ale neposkytuje ¾ádné propojení s ji¾ existujícími
objekty. Z tìchto dùvodù se vývojáøi projektu FreeIPA rozhodli vytvoøit vlastní
schéma pro uchovávání sudo pravidel na IPA serveru. Toto schéma umo¾òuje lep¹í
...
specifikován pouze jako øetìzec znakù, ale jako odkaz na existující identitu
dané domény. 

{\color{red}{
TODO propojení pravidel s identitami (pokud sma¾u u¾ivatele tak u¾ jeho pravidlo
nepotøe- buji, na IPA se sma¾e, ale v èistém LDAPu by zùstalo) 
}}

Schéma definuje novou tøídu pro sudo pravidla, jejich¾ definice vypadá
následovnì:

\lstinputlisting{listings/ipasudorule}

Tøída \texttt{ipaSudoRule} dìdí atributy tøídy \texttt{ipaAssociation}. Ka¾dé
záznam, popisující konkrétní sudo pravidlo, obsahuje jedineèné ID, své jméno a
dále mù¾e obsahovat atributy specifikující pravidlo.

Obecnì v záznamech sudo pravidel platí, ¾e \texttt{member*}
atributy\footnote{tj. napøíklad \texttt{memberUser, memberAllowCmd} a dal¹í}
obsahují DN ji¾ existujících objektù IPA domény, zatímco
\texttt{external*}\footnote{tj.  napøíkad \texttt{externalUser, externalHost} a
dal¹í} atributy specifikují objekty, které jsou definovány mimo IPA doménu.
Speciální pøípady, kdy je napøíklad sudo pravidlo aplikovatelné na kteréhokoliv
u¾ivatele nebo poèítaè specifikují \texttt{*Category} atributy.

U¾ivatele, na kterého bude pravidlo aplikovatelné, je mo¾né specifikovat pomocí
tøí atributù. Pøíklady hodnot popisuje tabulka \ref{table:ipa_user_attrs}. Jak
je mo¾né odvodit z prvního øádku této tabulky, záznamy u¾ivatelù IPA domény se
nacházejí v kontejneru \texttt{ cn=users,cn=accounts,\$DC}. Pomocí
\texttt{externalUser} atributu je také mo¾né vytvoøit sudo pravidlo
aplikovatelné na u¾ivatele, který není èlenem IPA domény. Tento u¾ivatel ov¹em
musí být èlenem jiné domény\footnote{napøíklad doména spravována pomocí Active
Directory} a mezi touto a IPA doménou musí být vytvoøena vzájemná dùvìra, tzv.
\textit{trust}. Popis jak toto propojení mezi doménami funguje je ov¹em nad
rámec této bakaláøské práce.
...
\begin{table}[h]
  ...
  \begin{tabular}{| l | l |} \hline
    ...
    memberUser & uid=xsruba03,cn=users,cn=accounts,\$DC\footnote{Domain Controller} \\ \hline
    userCategory & all \\ \hline
    externalUser & xsruba04 \\ \hline
  \end{tabular}
  \end{center}
...
identitu, tak i skupinu.

Stejným zpùsobem je poté definován poèítaè na který bude pravidlo aplikováno.
Zde je navíc definován atribut \texttt{hostMask}. Pomocí nìho¾ je mo¾né
specifikovat poèítaè pomocí adresy sítì. Tento atribut ov¹em v praxi není
vyu¾íván. 

Poslední mno¾inou, kterou je nutno specifikovat pro vytvoøení kompletního
pravidla, jsou pøíkazy. Zde definované schéma pøiná¹í novou vlastnost a tou jsou
...
pøíklad jejich mo¾né hodnot popisuje tabulka \ref{table:ipa_sudo_cmds_attrs}.
...
\begin{table}[ht]
  ...
  \begin{tabular}{| l | l |} \hline
    \textbf{Jméno atributu} & \textbf{Hodnota atributu} \\ \hline\hline
    memberAllowCmd & cn=dics,cn=sudocmdgroups,cn=sudo,\$DC \\ \hline
    memberDenyCmd & ipaUniqueID=41e9a39c,cn=sudocmds,cn=sudo,\$DC \\ \hline
    cmdCategory & all \\ \hline
  \end{tabular}
  \end{center}
	\caption[Table caption text]{Atributy specifikující sudo pøíkaz}
	\label{table:ipa_cmds_attrs}
\end{table}

Kromì skupin pøíkazù pøiná¹í nové schéma dal¹í výhodu a tou je
povolování/zakazování pravidel. Jestli¾e administrátor spravuje sudo pravidla
pomocí LDAP serveru. Pak pravidla, která jsou v adresáøi definována budou v¾dy
pou¾ita. Pokud by se rozhodl, napøíklad z dùvodu testování, nìjaké pravidlo
...
jeho odstranìní z adresáøe. Pøíklad záznamu sudo pravidla\ref{fig:sudo_rule} na
IPA serveru znázoròuje obrázek \ref{fig:ipa_sudo_rule}.
...
IPA klienty o \textbf{modifikaci} jakéhokoliv záznamu \cite[s.~130, Sekce:
Tracking Modifications to Directory
Entries]{Redhat_Directory_Server_Administration_Guide}. 

Kompletní schéma, které popisuje tøídy a atributy pro definici sudo pravidel a
pøíkazù je mo¾né nalézt na serveru FreeIPA v
\texttt{/etc/dirsrv/slapd-instance\_name/65sudo.ldif}. Schémata, která pou¾ívá
FreeIPA server je mo¾né najít také na pøilo¾eném DVD, viz. \ref{dvd}.

\subsection{Definice sudo pravidel} Sudo pravidla na IPA serveru je mo¾né
definovat dvìma zpùsoby. Jednak pomocí webového rozhraní nebo pomocí utilit
pøíkazového øádku \texttt{ipa sudorule*}. Podrobnìj¹í popis s pøíklady jak
definovat pravidla ve webovém rozhraní nebo pomocí pøíkazového øádku je mo¾né
nalézt napøíklad v oficiální dokumentaci\footnote{Red Hat Enterprise Linux 6
Identity Management Guide}. Pøi správì pravidel v LDAP adresáøi bylo nutné
pravidla vytváøet textovì a ruènì je do adresáøe pøidávat. Webové rozhraní zde
tedy administrátorovi velice zjednodu¹uje práci.  

{\color{red}{
TODO pøídat definici toho pravidla, které pou¾ívá¹?
}}

%-------------------------------------------------------------------------------
\section{System Security Services Daemon}
SSSD je daemon jeho¾ primární funkcí je poskytovat pøístup k identitám nebo
politikám ulo¾ených na vzdáleném serveru skrze framework, který poskytuje
cachování a offline podporu.
...
separátním úètem, který mu umo¾nil pøístup do systému. SSSD umí cachovat
informace o identititách a jejich autorizaci. Pokud se klient bude chtít
...
pouze z pohledu sudo utility. Tedy jakým zpùsobem SSSD zpracovává sudo pravidla
ne¾ jsou pøedány sudo utilitì.
...
{\color{red}{TODO: s SSSD mù¾e¹ pøistupovat k pravidlum v ldapu}}
...
mo¾né tento typ aktualice najít jako \uv{\textit{smart refresh}}.
...
mo¾né tento typ aktualice najít jako \uv{\textit{full refresh}}.
...
\texttt{man sssd.conf}}. Jestli¾e se nìkteré pravidlo na serveru
...
najít jako \uv{\textit{rules refresh}}. 
...
providera.
{\color{red}{TODO: zmaìruju se na sudo pravidla na IPA serveru!}}
...
Tento provider provádí ve¹kerou práci pøi komunikaci s IPA serverem na kterém jsou
ulo¾ena sudo pravidla. Sta¾ení, zpracování a ulo¾ení tìchto pravidel do sysdb.
Skládá z nìkolika modulù jejich¾ celkovou funkcionalitu
...
{\color{red}{TODO: pøidat to jak nastvuje¹ typ atributu a podle toho rozhode¹
jak jej pøelo¾it}}
...
\begin{itemize}
	\item literaturu oèisluj, tak jak ji pou¾ívá¹
	\item emph instead of textit? you have some note about it at wiki ...
