...
Tento atribut je pou¾íván pøi provádìní prùbì¾ných aktualizací\footnote{Smart
Refreshes} sudo pravidel. Implementaèní detaily je mo¾né nalézt v sekci
...
posldní aktualizace modifikovány.
...
\newline
\\
Implementaèní detaily jsou popsány v sekcích 
\ref{ch:sssd:sec:ldap_provider:smart_refresh_implementation},
...
\begin{itemize}
	\item alokace \texttt{sudo\_ctx}, který je zároveò privátnimi daty LDAP SUDO
			Providera
	\item nastavení handleru pro SUDO respodera
	\item vytvoøení mapy z výchozí \texttt{native\_sudorule\_map} mapy
	\item získání v¹ech jmen a IP adres daného poèítaèe
\end{itemize}
	
Pøi inicializaci se také inicializuje struktura bet\_ops, kde se nachází SUDO
handler. 
...
\begin{itemize}
	\item	V¹echny tøi typy aktualizací vyu¾ívají modul
		\texttt{providers/ldap/sdap\_async\_sudo.c}, který provádí
		\ref{async_engine_of_ldap_provider}
	\item v¾dycky se akorát pøedpøipraví paramaetry jako je jsou LDAP filter,
		sysdb filter
	\item pøi ka¾dém zavolání sudo se provede RULES refresh, pokud jsou v sysdb
		nìjaká expirovaná pravidla.
	\item pokud mám v cache pravidlo cn=test1 (povolující spustit cat /etc/shadow)
		a cn=test2 (povolující spustit /sbin/blkid). cn=test1 je v dobì spu¹tìní
		sudo expirováno. Pokud spustím sudo /sbin/blkid, tak se mnì provede RULES
		refresh pravidla cn=test1!!!
\end{itemize}
...
\begin{itemize}
	...
	\item filter, který LDAP SUDO Provider pou¾ívá pro FULL refresh:
...
\subsection {Prùbì¾ná aktualizace}\label{ch:sssd:sec:ldap_provider:smart_refresh_implementation}
\begin{itemize}
	\item	smyslem tohoto typu aktualizace je prùbì¾nì stahovat z IPA pravidla,
		která tam pøibyla od poslední aktualizace
	\item	pou¾ívá se k tomu EntryUSN attr, který zachytí i novì enabled pravidla
	\item nezachytí to smazání pravidla, pouze modifikaci stávajícího nebo pøidání
		nového
	\item pou¾ívá se tento filtr
\end{itemize}
...
\subsection {Asychnronní sta¾ení pravidel}
\label{async_engine_of_ldap_provider}
\begin{itemize}
...
Pøed samotným ulo¾ením pravidel jsou z cache pamìti nejprve vyhledány a následnì
odstranìny v¹echny záznamy, které spadají pod tøídu \texttt{sudoRule}.  Pøi
ukládání pravidel se ka¾dému pravidlu pøidá atribut
...
\texttt{cn} atributù}. SUDO responder mù¾e po¾ádat o kompletní aktualizaci
