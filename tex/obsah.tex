...
nedostatky, které tato práce odkrývá. Smyslem této práce je tedy tyto nedostatky
...
online adresáøù a LDAP protokolu. Tato práce byla zadána ve spolupráci
s~firmou Red Hat.
...
FreeIPA, které se tyto nedostatky sna¾í øe¹it. Dále popisuje klientského démona
...
systému, neboli u¾ivatel \emph{root}, tedy u¾ivatel, který má nejvy¹¹í oprávnìní.
...
nároèné. Otázkou tedy zùstává jak uvedená pravidla ¹íøit mezi více poèítaèù.
...
do LDAP adresáøe je tedy velice vhodné a nabízí následující výhody:
...
pravidel. Schéma tedy definuje, které atributy je nutno pou¾ít pro ulo¾ení
prvkù\footnote{Na koho bude pravidlo aplikovatelné, kde, jaké obsahuje pøíkazy,
\dots} SUDO pravidla.  Definuje také tøídu, ze které jsou záznamy,
...
SUDO pøíkaz je také mo¾né spustit, jako jiný u¾ivatel ne¾ \texttt{root}. Specifikovat
...
Schéma sudo pravidel také definuje atributy \texttt{sudoNotBefore} a
...
validní\footnote{napø. po dobu pracovní doby od 7:00 do 15:00}. Tyto atributy
...
\texttt{xsruba03}\footnote{je tedy definován v~souboru \texttt{/etc/passwd}} a
...
Projekt FreeIPA\footnote{zkrácenì jen IPA} se zamìøuje na správu identit a
...
jiné domény\footnote{napøíklad doména spravována pomocí Active Directory} a mezi
...
pluginu\footnote{Jedná se o~plugin, který dodává SUDO. Nezamìòovat tedy s~ldap
...
jednak offline podporu, ale také mo¾nost ukládání tìchto pravidel do cache pamìti.
Konfigurace SSSD pro tento úèel je popsána v~manuálové stránce
...
na SUDO providera a zásuvné moduly, které má k~dispozici. Tento popis byl
sestaven na základì experimentování a ètení zdrojových kódù. Získání zde
popsaných informací bylo pøitom nezbytné pro úplné pochopení zpùsobu, jakým SSSD
SUDO pravidla zpracovává. V~pøedposlední sekci je také popsán hlavní problém, který
je v této práci øe¹en a to podpora IPA SUDO schéma v démonu SSSD.
...
Knihovna \emph{tevent} \cite{tevent} realizuje tyto události za pomocí
asynchronních funkcí.  Synchronní funkce poskytují programátorovi pouze jedno
rozhraní, pøes které je mo¾no specifikovat vstupní parametry funkce a získat
výsledek dané funkce.  Naproti tomu asynchronní funkce poskytují rozhraní dvì.
Jedno pro pøedání parametrù funkci a získání výsledkù a druhé pro zaslání
samotného po¾adavku a uvìdomìní programátora o~dokonèení této asynchronní
události. Funkce, která je zavolána jako výsledek o~dokonèení události se nazývá
tzv. \emph{callback}.  
...
funkci je mo¾né zavolat pomocí funkce \uv{request\_send}\footnote{funkci je
mo¾né si pojmenovat libovolnì, napø. \texttt{ldap\_query\_send}}. Poté probíhá
asynchronní provádìní programu. V~okam¾iku kdy bude událost oznaèena jako
dokonèená\footnote{napø. pomocí funkce \texttt{tevent\_req\_done()}}, vyvolá se
její callback tj. funkce \uv{request\_done}. V~této funkci je mo¾né zpracovat
výsledky dané události a následnì ji odstranit.  
...
\label{ldap_sudo_provider_initialization}
Inicializaci zaji¹»uje modul \texttt{providers/ldap/sdap\_sudo.c} a probíhají
pøi ní následující úlohy:
\begin{itemize}
	\item alokace a inicializace privátních dat LDAP SUDO providera,
...
Inicializaci provádí funkce \texttt{sdap\_sudo\_init()}. 
%Vytváøí se zde
...
pomocí
voleb \texttt{ldap\_sudo\_hostnames} a \texttt{ldap\_sudo\_ip}. Jejich hodnoty
...
Pøed ulo¾ením sta¾ených pravidel jsou nejprve ze sysdb odstranìny \textbf{v¹echny} SUDO
...
generuje velký sí»ový provoz. Provádí se tedy pouze pøi spu¹tìní SSSD, pøi
detekci zmìny pravidel na serveru nebo periodicky s~men¹í frekvencí ne¾ prùbì¾né
aktualizace. Pøi zachování výchozího nastavení se tato aktualizace provádní
...
plugin v~sekce \ref{ipa_sudo_schema}.
...
Pøedpokládejme pøíklad, kdy v~sysdb ji¾ nejsou validní pravidla \textbf{rule1} a
\textbf{rule2}. Nejprve se provede vyhledání tìchto pravidel na serveru. Pro
LDAP dotaz bude pou¾it filtr \ref{ldap:rules_refresh}. Jakmile budou potøebná
...
\ref{sysdb:rules_refresh}. Novì sta¾ená pravidla poté budou ulo¾eno do sysdb.
\\\\
\def\lstlistingname{LDAP filtr}
\lstset{
	language=bash,
...
pluginu, který bude zaji¹»ovat podporu nativního IPA SUDO schématu. Návrhem
nativního IPA SUDO providera se zabýva kapitola
\ref{ch:native_ipa_sudo_provider}.

\section{Dereferenèní dotaz}\label{sec:deref}
LDAP dotaz umo¾òuje získat záznam, popøípadì mno¾inu záznamùz LDAP adreáøe.
Jestli¾e se nìjaký atribut z~tìchto záznamu odkazuje na jiné záznamy (hodnota
nìkterého z~atributù obsahuje DN jiného záznamu v~adresáøi). Pak pro získání
tìchto záznamù je nutné vytvoøit nový dotaz.
...
Chování API knihovny OpenLDAP je mo¾né mìnit pomocí tzv. \uv{\emph{controls}}. Je
tedy mo¾né modifikovat chování funkce \texttt{ldap\_search\_ext()}. Pak bude mo¾né
pomocí této funkce získat nejenom po¾adované záznamy, ale také atributy ze
záznamù, na které se daný záznam odkazuje. SDAP rozhraní vyu¾ívá této
funkcionality a nabízí programátorovi asynchronní funkci
\texttt{sdap\_deref\_search\_send()}. Tato situace je zachycena na obrázku
\ref{fig:sdap_deref}.

Server ov¹em musí tento typ \uv{controls} podporovat\footnote{supportedControl:
1.3.6.1.4.1.4203.666.5.16}. U~Active Directory se tato technika oznaèuje jako
\emph{Attribute Scope
...
bylo nutné získat pomocí dereference odkazù\footnote{Tato technika je blí¾e
...
metody \ref{all_at_once}, tzn.  sta¾ení SUDO pravidel a zároveò v¹ech pøíkazù v~jediném
LDAP dotazu. SSSD pou¾ívá své SDAP API pro provádìní LDAP dotazù. Výsledky LDAP
a SDAP dotazù ov¹em v~nìkterých pøípadech nejsou ekvivalentní\footnote{dùvodem
jsou mapy a záznamy, které nemají spoleèné tøídy}. Záznamy, pøijaté ze serveru,
jsou filtrovány pøedtím, ne¾ jsou pøedány programátorovi. To zaji¹»uje funkce
\texttt{sdap\_parse\_enrty}. Ta vrací pouze záznamy, které spadají do jedné
konkrétní tøídy.  Jestli¾e pøijaté záznamy nejsou odvozeny z~této konkrétní
tøídy, pak jsou zahozeny. 

Záznamy o~sudo pravidlech a pøíkazy pro tyto pravidla jsou ulo¾eny v~separátních
kontejnerech a také jsou definovány pomocí jiné tøídy, viz. Tabulka
