...
\begin{figure}[H]
	...
	\caption{Ukázka mapování atributù z sssd.conf do sysdb}
...
pluginu. Zde by také bylo mo¾né vyu¾ít atributu \texttt{ipaUniqueID}, který jednoznaènì
identifikuje sudo pravidlo. To by ov¹em vy¾adovalo úpravu sudo respondera, která by
nepøinesla ¾ádnou výhodu. Jeliko¾ jsou \texttt{cn}, tj. jména sudo pravidel, 
jedineèná i v IPA SUDO formátu, není potøeba tuto zmìnu provádìt. Pøi prùbì¾ných
aktualizacích je opìt vyu¾ito atributu \texttt{EntryUSN}.
...
pravidla neobsahují odkazy na dal¹í záznamy, tj. záznamy pøíkazù. Zva¾ována byla také varianta kdy by
se pro úplnou aktualizaci   postupovalo podle metody \ref{all_at_once}, tzn.
sta¾ení sudo pravidel a v¹ech pøíkazù v jediném LDAP dotazu. Zde ov¹em vzniká
problém s SDAP rozhraním, které neumo¾òuje specifikovat více ne¾ jednu mapu.
Proto není mo¾né obdr¾et záznamy, které nemají alespoò jednu stejnou tøídu.
Tento problém je blí¾e popsán v sekci \ref{sdap}. Sudo pravidla v IPA SUDO
schématu mají tøídu \texttt{ipaSudoRule}, zatímco pøíkazy \texttt{ipaSudoCmd}. 
...
implementaèního hlediska by tyto rozhraní pro daný úèel bylo mo¾né upravit. Je
otázkou zda by výhody této úpravy pøedèily èas stráveny nad touto zmìnou a
mo¾nost zanesení chyby do velice èasto vyu¾ívané a odladìné èásti kódu.
...
Pro prvotní sta¾ení pravidel byl vyu¾it LDAP plugin. To vy¾adovalo úpravu, která
je zobrazena na schématu \ref{sdap_async_sudo_ipa_change}. Výsledné schéma
asynchronních událostí ipa pluginu poté zobrazuje schéma
\ref{ipa_plugin_tevents_flow}. Z tohoto modulu jsou dále vyu¾ity funkce, které
zaji¹»ují práci s cache pamìtí, tzn. odstraòování expirovaných a ukládání novì
sta¾ených a pøelo¾ených pravidel. Plánování aktualizací je takté¾ èást, kterou
mají oba pluginy spoleènou.
