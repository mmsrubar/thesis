...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\chapter{Správa bezpeènostních politik identit} Tato kapitola popisuje mo¾nost
správy bezpeènostních politik identit pomocí pomocí programu sudo. Popisuje
mo¾né problémy, které mohou nastat pøi distribuci pravidel, kterými se program
sudo øídí a dále LDAP schéma, které sudo pou¾ívá pro ulo¾ení tìchto pravidel
v~LDAP adresáøích. Zabývá se nedostatky, kterými toto schéma disponuje z~pohledu
domény a pøedstavuje schéma projektu FreeIPA, které se tyto nedostatky sna¾í
øe¹it.  Dále popisuje klientského démona SSSD, který je nezbytný pro vyu¾ití
tohoto schématu s~programem sudo.
...
\subsection{Nativní LDAP sudo schéma}\label{native_ldap_sudo_schema}
Sudo má vlastní podporu pro ulo¾ení sudo pravidel do LDAP adresáøe. Definuje
také schéma, které je nutno dodr¾et. Tøída \texttt{sudoRole}, která
...
ov¹em nedoporuèuje pou¾ívat a pova¾uje se jako \uv{zavr¾ený}\footnote{deprecated}.
...
\begin{itemize}
	...
	\item Pøi ¹patné konfiguraci NSS\footnote{Name Service Switch} mù¾e dojít
k~pøekrytí u¾ivatele z~LDAP adresáøe s~lokálním u¾ivatelem.
\end{itemize}
...
objekty dané domény. Z~tìchto dùvodù se vývojáøi projektu FreeIPA rozhodli vytvoøit vlastní
schéma pro uchovávání sudo pravidel. Toto schéma umo¾òuje lep¹í
integraci s~ji¾ existujícími objekty s~konkrétními pravidly. Umo¾òuje
...
v~samostatných kontejnerech. Jsou také definovány pomocí jiných tøíd. Atributy a
...
\texttt{/etc/dirsrv/\allowbreak slapd-instance\_name/}. Detailní popis tøíd a atributù pro
definici sudo pravidel na IPA serveru je mo¾né nalézt v~souboru \texttt{65sudo.ldif}.
Schémata, která pou¾ívá FreeIPA server je mo¾né najít také na pøilo¾eném CD,
viz. Pøíloha \ref{dvd}.
...
pøi detekci zmìny pravidel na serveru. V~manuálnových stránkách a v~kódu SSSD
je mo¾né tento typ aktualizace najít jako \uv{\emph{full refresh}}.
...
Po spu¹tìní programu sudo se nejprve zjistí informace o~u¾ivateli, který program sudo
vyvolal. Knihovna NSS po¹le po¾adavek
o~informacích  NSS responderu, tj. procesu sssd\_nss. Ten nejprve zjistí zda se
...
dodr¾et sudem definované schéma, které bylo popsáno v~sekci
\ref{native_ldap_sudo_schema}. Tedy pøesná jména atributù a formát hodnot.
...
vlastní asynchronní rozhraní \texttt{sdap\_get\_generic\_ext\_send()}, které
s~tìmito mapami pracuje. Toto rozhraní také provádí zpracování výsledkù, které
...
uvedena tøída.  Pøijaté záznamy, které neodpovídají této
tøídì nebudou pøedány jako výsledek!  Takté¾ v~mapì není mo¾né specifikovat více
ne¾ jednu tøídu! Výsledky \texttt{ldap\_search\_ext} a
...
v~konfiguraèním souboru SSSD.  Ukazatel na vytvoøenou mapu je ulo¾en
v~\texttt{id\_ctx}\footnote{Kontext providera identit (angl.  Context of Identity
Provider)}. Vytvoøení a pou¾ití map je blí¾e popsáno v~sekci \ref{sdap}.
...
filter. Tento i v¹echny následující filtry se øídí syntaxí specifikovaou LDAP
protokolem \cite[RFC2254]{rfc2254}.  Jsou pouze doplnìny bílými znaky pro lep¹í
èitelnost.

\lstinputlisting{CD/ldap_filters/ldap_sudo_provider/full_refresh_filter}
...
\lstinputlisting{CD/ldap_filters/ldap_sudo_provider/smart_refresh_filter}
...
Jestli¾e se v~sysdb pøi spu¹tìní programu sudo nachází nìjaká pravidla, která ji¾ nejsou
...
\lstinputlisting{CD/ldap_filters/ldap_sudo_provider/rules_refresh_filter}
...
\texttt{sssd.conf}}. Jeho hodnota reprezentuje èas, kdy daný záznam stane
...
zvá¾it pøi návrhu, implementaci a testování nativního IPA SUDO providera. Tento
provider poté bude podporovat nativní IPA SUDO schéma pou¾ívané FreeIPA
serverem. Návrh popisuje mo¾né pøístupy ke sta¾ení a zpracování sudo pravidel,
jejich¾ výhody a nevýhody byly diskutovány s~vývojáøi LDAP SUDO providera.
Nìkteré specifické detaily také s~vývojáøi projektu FreeIPA. Sekce implementace
popisuje jak a které z~tìchto pøístupù byly vybrány pro implementaci nového
providera. V~sekci testování je poté popsána metodika a nástroje vyu¾ité
k~otestování navr¾ených øe¹ení.
...
Jeliko¾ program sudo doká¾e zpracovat pouze pravidla, která odpovídají nativnímu LDAP
...
pravidel) ze kterých by bylo mo¾né pøes odkazy získat odkazované záznamy (sudo pøíkazy).
Z~implementaèního hlediska by tyto rozhraní pro daný úèel bylo mo¾né upravit. 
