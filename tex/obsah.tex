...
\subsubsection{problem spojené s tím, kdy¾ mám sudoers v LDAPu}
	\begin{itemize}
		\item problem s pøekrýváním u¾ivatelù
			Michichael-Sune> sudoers: ldap files
<jhrozek> ok, first..it should be 'files sss', not the other way around
<jhrozek> you don't want a 'root' user from LDAP shadow your local root
...
zvá¾it pøi návrhu, implementaci a testování nativního IPA sudo pluginu. Návrh
popisuje mo¾né pøístupy ke sta¾ení a zpracování sudo pravidel, jejich¾ výhody a
nevýhody byly diskutovány s vývojáøi ldap sudo pluginu. Nìkteré specifické
detaily také s vývojáøi projektu FreeIPA. Sekce implementace popisuje jak a
které z tìchto pøístupù byly vybrány pro implementaci nového pluginu. V sekci
...
IPA plugin bude provádìt velmi podobnou funkcionalitu jako LDAP plugin. Mohlo by
se tedy zdát, ¾e jediná úprava, kterou je nutno provést je pøidání pøekladu
pravidel pøed samotným ulo¾ením tìchto pravidel do sysdb. Pøevod pravidel je
ov¹em  specifiký pro IPA providera. V kódu LDAP pluginu by se tedy muselo
vyskytnou volání funkcí z IPA kódu. Tento pøístup je sice
...
byl souèástí dynamické knihovny \texttt{libsss\_ipa.so} zatímco kód LDAP pluginu
...
sudo provider tedy vyu¾ije  LDAP plugin z knihovny \texttt{libsss\_ldap.so}. Knihovna
...
která není naètena v pamìti. Proto není mo¾né z LDAP pluginu volat kód z IPA
pluginu. Je ov¹em mo¾né z IPA pluginu volat kód LDAP pluginu, LDAP plugin je
mo¾né si pøedstavit jako stavební blok nad kterým mohou stavìt
odstatní provideøi a toho je mo¾no vyu¾ít. Vet¹í zásah do LDAP pluginu by také
...
ulo¾ení sudo pravidel do sysdb.
...
specifikuje klienta pomocí doménového jména  a skupin poèítaèù ve
kterých je èlenem. Pro získání skupin ve kterých je klient èlenem bylo pou¾ito asynchronní
volání \texttt{ipa\_host\_info\_send()}. 

Kompletní sudo pravidla jsou pak tedy
...
\begin{itemize}
	...
	\item reálná sudo pravidla si firmy støe¾í jako oko v hlavì, tak¾e je 		nedostanu a proto jsem musel vytvoøit pouze umìlá ...
\end{itemize}
